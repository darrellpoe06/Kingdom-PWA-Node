...
SURFACES = os.path.join(ROOT, "app", "src", "surfaces.js")
...
def compute():
    ...
    # Surface-registry size = one lazy loader (`=> import(`) per mounted surface.
    # Counted HERE (not imported into a feature component) so the boards never
    # import the shell-only registry — module-boundary-guard (DR-0076) forbids it.
    surfaces = 0
    try:
        with open(SURFACES, "r", encoding="utf-8") as f:
            surfaces = f.read().count("=> import(")
    except OSError:
        surfaces = 0
