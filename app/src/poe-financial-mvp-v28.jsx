...
import { deriveBalances, accountBalance } from './lib/account-balances.js';
...
export const SEED_DATA = {
  ...
  accounts: [
    ...
    // ledger. The displayed "Right now" balance is DERIVED via lib/account-
    // balances.js: openingBalance + the sum of that account's cleared (settled,
    // date <= today) transactions. Seed openings are set so the derived "now"
    // matches the intended display on first load, then moves as real entries
    // clear. Chase ...8168 opens at -130.29 because its cleared seed history
    // nets +4,353.29 -> derived now = 4,223. Accounts with no seed transactions
    // open at their balance (cleared sum = 0 -> now = opening).
  ...
  transactions: [
    ...
    // that differ by window instead of one repeated constant. Part of the
    // ...8168 cleared ledger that derives its "Right now" balance.
...
function BooksTransactions({ data, entityFilter, setEntityFilter, currentDate, addTransaction, updateTransaction, deleteTransaction, ingestData = null, visibleEntities = null, visibleEntityIds = null }) {
  ...
  // DERIVED balances — every "Right now" figure recomputes from the underlying
  // ledger via the shared deriveBalances helper, never read from a stored
  // literal. Current balance = openingBalance + the sum of that account's
  // settled (date <= today) transactions, so any real entry added later flows
  // through here and moves every dependent figure (the primary card, the inline
  // per-row "(now $X)", and the 30/60/90 forecast). See lib/account-balances.js
  // for the model and its proven-to-catch tests.
  const balanceByAccount = deriveBalances(data.accounts, data.transactions, todayISO);
  ...
  const forecast = (() => {
    ...
    cashAccounts.forEach(a => {
      ...
      // settled history), not the stored literal, so the forecast moves with the
      // ledger and ties out to the primary card's "Right now".
      const now = accountBalance(a, data.transactions, todayISO);
      perAccount[a.id] = {
  ...
  return (
    <div className="space-y-6">
      ...
      {(() => {
        ...
        // "Right now" is DERIVED from this account's settled ledger, not the
        // stored literal. Then project after all upcoming charges that hit it.
        const rightNow = balanceByAccount[primary.id] !== undefined ? balanceByAccount[primary.id] : accountBalance(primary, data.transactions, todayISO);
