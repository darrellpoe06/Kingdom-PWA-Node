// =============================================================================
// L160 — Pride is not worth Him; and is ego pride, or something else?
// Verbatim KJV, and the rules this lesson is bound by.
// =============================================================================
// Darrell 2026-09-15, spoken: "Pride is not worth Him!!!" "Lesson!!!" then
// "Is ego pride or something else entirely different?" "Lesson".
//
...
// (child, youth, teen, senior — DR-0418), quiz, facilitator notes. The ego
// question is answered from the Word (Romans 12:3; 1 Corinthians 4:7; John
// 13:3-5), never from the culture, and the gate pins the distinction.
...
const ID = 'll160-pride-is-not-worth-him-and-is-ego-pride-or-something-else';
...
describe('L160 is in the series, whole', () => {
  it('exists, follows L159, and the painted count is real', () => {
    expect(start).toBeGreaterThan(-1);
    expect(m).toBeTruthy();
    const at = LIVING_LESSONS_MODULES.findIndex((x) => x.id === ID);
    expect(LIVING_LESSONS_MODULES[at - 1].id.startsWith('ll159-')).toBe(true);
  ...
  it('carries the full teaching shape', () => {
    ...
    expect(m.anchor.ref.startsWith('Proverbs 16:18; Proverbs 8:13; James 4:6')).toBe(true);
  });
...
describe('Darrell’s word is in the lesson (CLAUDE.md: always add it)', () => {
  it('the directive: pride is not worth Him, and the ego question', () => {
    expect(m.lesson).toMatch(/THE DIRECTIVE\. Darrell, 2026-09-15, spoken/);
    expect(m.lesson).toMatch(/pride is not worth Him\. Lesson\./);
    expect(m.lesson).toMatch(/is ego pride, or something else entirely different\? Lesson\./);
    expect(m.lesson).toMatch(/FIRST, PRIDE IS NOT WORTH HIM\./);
    expect(m.lesson).toMatch(/SIXTH, IS EGO PRIDE, OR SOMETHING ELSE\?/);
    expect(m.lesson).toMatch(/EIGHTH, THE MIND OF CHRIST, WHO EMPTIED HIMSELF\./);
  });
  it('the ego answer is the Word’s: a sober measure is commanded; pride is the self above its measure; the towel', () => {
    expect(m.lesson).toMatch(/Sober thinking about yourself is a command\./);
    expect(m.lesson).toMatch(/Pride is the self lifted above its measure and cut loose from its Maker/);
    expect(m.lesson).toMatch(/The fullest self-knowledge in history produced the lowest service in history\./);
    expect(m.lesson).toMatch(/Pride is that knowledge without the towel\./);
    for (const ref of ['Proverbs 16:18', 'Proverbs 8:13', 'James 4:6', '1 Peter 5:5', 'Isaiah 14:14', 'Ezekiel 28:17', 'Genesis 3:5', 'Daniel 4:30', '2 Chronicles 26:16', 'Deuteronomy 8:17', 'Luke 18:14', 'Proverbs 13:10', 'Romans 12:3', '1 Corinthians 4:7', 'John 13:5', 'Luke 9:23', 'Philippians 2:8', 'Isaiah 57:15', 'James 4:10', '1 Samuel 15:17']) expect(m.lesson).toContain(`(${ref})`);
  });
});
...
describe('every age band is served text authored for IT (DR-0418: youth included)', () => {
  ...
  it('the child level teaches the real thing: the price, the first pride, the king, the two men, ego measured, the towel', () => {
    const c = m.levels.child;
    expect(c).toMatch(/Pride is not worth Him\./);
    expect(c).toMatch(/James 4:6/);
    expect(c).toMatch(/Daniel 4:30/);
    expect(c).toMatch(/Luke 18:14/);
    expect(c).toMatch(/Knowing who you are is not pride\./);
    expect(c).toMatch(/Pride is knowing who you are and refusing the towel\./);
...
describe('NO in-quote alteration — the whole-span gate (DR-0076)', () => {
  ...
  it('the anchors are quoted whole, letter for letter, with the reference beside each', () => {
    for (const [book, ch, v, label] of [['Proverbs', 16, 18, 'Proverbs 16:18'], ['James', 4, 6, 'James 4:6'], ['Isaiah', 14, 14, 'Isaiah 14:14'], ['Daniel', 4, 30, 'Daniel 4:30'], ['2 Chronicles', 26, 16, '2 Chronicles 26:16'], ['Luke', 18, 14, 'Luke 18:14'], ['Romans', 12, 3, 'Romans 12:3'], ['1 Corinthians', 4, 7, '1 Corinthians 4:7'], ['John', 13, 5, 'John 13:5'], ['Philippians', 2, 8, 'Philippians 2:8'], ['James', 4, 10, 'James 4:10']]) {
      const file = book.replace(/ /g, '');
      expect(m.lesson, label).toContain(`"${verse(file, ch, v)}" (${label})`);
    }
    expect(m.anchor.theme).toContain(`"${verse('Proverbs', 16, 18)}" (Proverbs 16:18)`);
    expect(m.anchor.theme).toContain(`"${verse('Romans', 12, 3)}" (Romans 12:3)`);
  });
  it('PROVEN-TO-CATCH: one altered word inside a quote fails the span gate', () => {
    const tampered = l.replace('Pride goeth before destruction, and an haughty spirit before a fall', 'Pride goeth before destruction, and a haughty spirit before a fall');
    const { spans } = quotedSpans(tampered);
    expect(spans.filter((s) => s.length >= 8 && !WHOLE_KJV.includes(s) && s.includes('and a haughty spirit')).length).toBeGreaterThan(0);
  });
});
...
describe('the house rules this lesson is bound by (CLAUDE.md, DR-0210, DR-0076)', () => {
  ...
  it('the close is the towel, tonight', () => {
    expect(m.lesson).toMatch(/do one unseen act of service before you sleep\./);
  });
});
