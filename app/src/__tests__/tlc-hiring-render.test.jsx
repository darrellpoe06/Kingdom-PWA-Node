...
const sent = { applications: [], jobs: [], reviews: [], hires: [], marks: [] };
...
vi.mock('../lib/tlc-hiring-sync.js', async (orig) => {
  ...
  return {
    ...
    applyToJob: async (jobId, applicant) => { const v = validateApplication(applicant); if (!v.ok) return { ok: false, reason: 'invalid', message: Object.values(v.errors)[0], errors: v.errors }; sent.applications.push({ jobId, applicant: normalizeApplication(applicant) }); return { ok: true, receipt: { id: 'app9', job_title: JOBS.find((j) => j.id === jobId).title, received_at: '2026-09-10T15:00:00Z' } }; },
...
afterEach(async () => {
  ...
  root = container = null;
  sent.applications.length = 0; sent.jobs.length = 0; sent.reviews.length = 0; sent.hires.length = 0; sent.marks.length = 0;
