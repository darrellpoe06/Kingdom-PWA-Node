...
describe('learn-resume — record and read the place', () => {
  it('round-trips a full place', () => {
    ...
    expect(getPlace({ storage })).toEqual({ courseKey: 'living-lessons', lessonId: 'll3', sentence: 0, sentenceKey: '', stage: 1, step: 2, at: 1000 });
  });
  ...
  it('merges a partial patch into the SAME lesson (a step move keeps the stage)', () => {
    ...
    expect(getPlace({ storage })).toEqual({ courseKey: 'living-lessons', lessonId: 'll3', sentence: 0, sentenceKey: '', stage: 1, step: 4, at: 2000 });
  });
  ...
  it('opening a DIFFERENT lesson resets stage/step — a stale step never leaks into a fresh lesson', () => {
    ...
    expect(getPlace({ storage })).toEqual({ courseKey: 'living-lessons', lessonId: 'll7', sentence: 0, sentenceKey: '', stage: 0, step: 0, at: 2000 });
  });
