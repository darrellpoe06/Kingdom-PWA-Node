...
// Learn framework (12-week set, computed timeline, multi-modal fields), teach the
...
describe('curriculum shape', () => {
  it('has the full 23-week module set (week 23: the swarm that agreed together and the plumbline held from outside, forwarded 2026-09-18)', () => {
    expect(SOVEREIGN_AI_MODULES).toHaveLength(23);
    expect(SOVEREIGN_AI_META.weeks).toBe(23);
...
describe('shared machinery (computed timeline, progress, export, cohort, tutor)', () => {
  ...
  it('progress is counted from the real record', () => {
    ...
    expect(r.pct).toBe(9);   // Math.round(2 / 23 * 100) === 9 (8.69… rounds to 9), same integer as 2 of 22
  });
