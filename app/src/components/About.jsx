  ...
  return (
    <div className="space-y-10 max-w-prose">
      <section>
        ...
        {/* 2026-06-02 reorder per Freddie-Taylor feedback (commit d3733f5) + the
            comprehensive tier-review (commit 4cb55b9). Lead with the FREE door so a
            new user sees "free forever" before any price. Freddie (a single adult)
        ...
        <div className="mb-5">
          ...
          {/* 2026-06-02 Foundation data-durability note per tier-review Decision 5c +
              DATA-AS-EMPOWERMENT-NOT-EXTRACTION (export is a commitment, not a paid
              feature). The export button is not wired yet, so the copy holds the
              promise honestly in the app's existing "in build" voice. */}
          <div className="mt-2 px-3 py-2 bg-[#FAF8F4] border-l-2 border-[#5A6E3D] text-xs text-[#5A5751]" style={{ fontFamily: '"Fraunces", serif' }}>
            <strong className="text-[#1A1815]">Your data is always yours.</strong> Free export (CSV / JSON / OFX) — no paywall, ever — is rolling out, with a gentle backup reminder every 30 days so a lost phone never means lost records.
          </div>
        </div>
        ...
        <div className="mb-5 px-4 py-3 bg-white border-2 border-[#B85838]">
          ...
          <p className="text-xs leading-relaxed" style={{ fontFamily: '"Fraunces", serif' }}>
            Two are <strong>free for everyone, forever</strong>: the <strong>Financial System for Families</strong> (the Foundation tier above) and the <strong>Spiritual Module for the edification of the Body</strong> (Scripture, the Godhead Study Platform, and worship surfaces — free on every tier, no upgrade). Two more open the door for those the work of justice names: <strong>Loved Ones · Founding Family</strong> (free PoeTech+ for life — the first 100 Church of the Living God families and chosen family invited directly by the Poe family) and <strong>Community · Families in Need</strong> (free access through partner churches and 501(c)(3)s, sponsored by paying subscribers). See the cards below the paid ladder.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {/* Round 5 — Tier features matched to the live tier-gating map (see
              ...
              per tier-review Decision 1; portals / sync / staff users are not live yet. */}
          <PricingTier name="PoeTech+" tagline="Tenant portal-ready · up to 3 properties · stop sharing your personal number" bestFor="a landlord with 1-3 doors, or a saver who wants cloud backup." monthly="39" annual="390" replaces="~$27-43/mo: paid YNAB tier ($15) + Stessa rental management for up to 3 doors ($12-28) + encrypted backup" availableNow={['Everything in Foundation','Real Estate — up to 3 properties, full edit (lease · tenant · equipment · rooms · maintenance · conversations · evaluator · map)','Unlimited entities in Books','Unlimited Markets watchlist','Dev/Ops — 3 personalized options per profile','Priority email support']} shipsLater={['🔐 Tenant portals — each tenant logs in via emailed link, sees only their lease + rent history + maintenance requests, messages you through a scoped channel (when external auth ships)','Cross-device sync (opt-in cloud)','Encrypted cloud backup']} onChoose={openCart} />
          <PricingTier name="Household" bestFor="unlimited rentals, or a solo pro (landlord / advisor) who needs Legal Matters + Projects." tagline="Unlimited tenant portals · multi-module use · Projects + Legal Matters" monthly="89" annual="890" replaces="~$80-150/mo at full build: budgeting ($15) + unlimited-property management with tenant portals (RentRedi $29 + Stessa Pro $28, or DoorLoop $99) + Notion-class ops ($10-12) + an encrypted document vault" availableNow={['Everything in PoeTech+ (available-now items)','🔒 Legal Matters tab — PIN + AES-GCM encrypted, attorney-client privilege flag on every note (export strips privileged content before non-counsel sharing)','Real Estate — unlimited properties','Projects tab — multi-domain timeline + workload + per-project conversations','Dev/Ops — full opportunity library (6+ matched options per profile)']} shipsLater={['🔐 Unlimited tenant portals — every door gets its own scoped channel; the tenant at one address never sees the tenant at another (when external auth ships)','Multi-user household sharing (opt-in, when backend ships)','Home Command Center module (when launched)','Seasonal maintenance calendar · IoT sensor pairing (planned)']} onChoose={openCart} />
      ...
      <section>
        ...
        {/* 2026-06-02 — Black Church economic-stewardship lens per Darrell's mission
            unification (docs/00-foundations/_root/BLACK-CHURCH-ECONOMIC-STEWARDSHIP.md).
            Names the first community; holds the soul-first order (3 John 1:2) and the
            faithfulness-not-yield metric (Matt 25:21) as the bright line vs prosperity gospel. */}
        <p className="text-base leading-relaxed mb-4" style={{ fontFamily: '"Fraunces", serif' }}>
          <strong>The Black Church is the first community PoeTech serves.</strong> Not as charity — as resourcing. From the Free African Society of 1787 to the colleges built on washerwomen's pennies, this is a documented economic powerhouse the mainstream finance and tech industries underserved for two centuries. The order is fixed: <em>prosper as your soul prospers</em> (3 John 1:2). The Spiritual Module for the Body is free on every tier; the financial tools follow. And the metric is faithfulness, not yield — the steward of two talents and the steward of five receive the same <em>"well done"</em> (Matthew 25:21). That is stewardship, not the prosperity gospel.
        </p>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <MarketCard title="Single-parent small business owners" need="One platform vs juggling SaaS, lead tracking, tax preparation" have="Premium tier — replaces ~$125-180/mo of CRM, intake, and scheduling SaaS" />
          ...
          <MarketCard title="Independent farmers & small homesteaders" need="Property + equipment + seasonal calendar + multiple income streams" have="Household tier covers this almost completely" />
