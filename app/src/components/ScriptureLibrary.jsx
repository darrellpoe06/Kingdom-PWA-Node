        ...
        <div className="mt-1.5 flex flex-wrap gap-1.5">
          {OTHER_VERSIONS.map((v) => (
            <a key={v.id} href={readOnline(refStr, v.id)} target="_blank" rel="noopener noreferrer"
              ...
              className="text-[10px] uppercase tracking-wider px-2 py-1 border border-[#E8E4DC] text-[#5A5751] hover:text-[#1A1815] hover:border-[#B85838] focus:outline focus:outline-2 focus:outline-[#B85838]">{v.label} ↗</a>
          ))}
        </div>
      )}
    </div>
  ...
  return (
    <div className="bg-white border border-[#E8E4DC] p-3">
      ...
      <p className="text-sm text-[#1A1815] mt-1 leading-relaxed" style={serif}>
        ...
        “{kjv}”<span className="text-[10px] text-[#5A5751] ml-1 align-baseline" style={mono}>KJV</span>
      </p>
      {gloss && <p className="text-xs text-[#5A5751] mt-1.5" style={serif}>{gloss}</p>}
      {(backs || []).length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-2">
          <span className="text-[9px] uppercase tracking-wider text-[#5A5751] py-0.5">Backs:</span>
          {surfaceLabels(backs).map((label) => (
            <span key={label} className="text-[9px] uppercase tracking-wider bg-[#FAF8F4] border border-[#E8E4DC] text-[#5A5751] px-1.5 py-0.5">{label}</span>
          ))}
        </div>
      )}
              ...
              <fieldset key={i} className="space-y-1">
                ...
                {graded && (
                  <p className={`text-xs mt-0.5 ${res && res.correct ? 'text-[#5A6E3D]' : 'text-[#5A5751]'}`} style={serif}>
                    {q.explain} {q.ref && <span style={mono} className="text-[10px]">({q.ref})</span>}
                  </p>
                )}
              </fieldset>
            );
    ...
    <section aria-labelledby={`thm-${theme.id}`} className="mb-8">
      ...
      {theme.lens && (
        <div className="bg-[#FAF8F4] border-l-2 border-[#5A6E3D] pl-3 pr-2 py-2 mb-2 space-y-1" style={serif}>
          <p className="text-xs text-[#1A1815]"><span className="uppercase tracking-wider text-[9px] text-[#5A6E3D]">His perspective · </span>{theme.lens.perspective}</p>
          <p className="text-xs text-[#1A1815]"><span className="uppercase tracking-wider text-[9px] text-[#5A6E3D]">His heart · </span>{theme.lens.heart}</p>
          <p className="text-xs text-[#1A1815]"><span className="uppercase tracking-wider text-[9px] text-[#5A6E3D]">His love · </span>{theme.lens.love}</p>
        </div>
      )}
      {theme.soul && (
        <p className="text-xs text-[#B85838] mb-2" style={serif}><span className="uppercase tracking-wider text-[9px]">For the soul · </span>{theme.soul}</p>
      )}
      ...
      <div className="flex flex-wrap gap-1.5 mb-2.5">
        ...
        {surfaceLabels(theme.surfaces).map((label) => (
          <span key={label} className="text-[9px] uppercase tracking-wider bg-[#5A6E3D] text-white px-1.5 py-0.5">{label}</span>
        ))}
      </div>
      ...
      <div className="bg-white border border-[#E8E4DC] p-3 mb-2">
        <div className="flex items-center justify-between gap-2 flex-wrap mb-1.5">
          <span className="text-[9px] uppercase tracking-wider text-[#5A5751]">Read at your depth · {DEPTH_TIERS.find((d) => d.id === depth.tierId)?.label}</span>
          <div className="flex gap-1" role="group" aria-label={`Depth for ${theme.title}`}>
            {DEPTH_TIERS.map((d) => (
              <button key={d.id} type="button" onClick={() => setLocalTier(d.id)} aria-pressed={depth.tierId === d.id}
                className={`text-[10px] uppercase tracking-wider px-2 py-1 border focus:outline focus:outline-2 focus:outline-[#B85838] ${depth.tierId === d.id ? 'bg-[#1A1815] text-white border-[#1A1815]' : 'bg-white text-[#5A5751] border-[#E8E4DC] hover:text-[#1A1815]'}`}>{d.label}</button>
            ))}
          </div>
        </div>
      ...
      {hasLevel && (
        <div className="bg-[#FAF8F4] border border-[#E8E4DC] p-3 mb-2">
          <span className="text-[9px] uppercase tracking-wider text-[#B85838]">For a {EXPERIENCE_LEVELS.find((l) => l.id === level)?.label.toLowerCase()}</span>
          ...
          <div className="space-y-2">
            {theme.views.map((view) => (
              <div key={view.name}>
                ...
                <p className="text-[10px] text-[#5A6E3D] mt-0.5" style={mono}>{(view.scriptures || []).join(' · ')}</p>
              </div>
            ))}
          </div>
  ...
  return (
    <div className="max-w-3xl">
      ...
      <blockquote className="border-l-2 border-[#5A6E3D] bg-[#FAF8F4] pl-3 pr-2 py-2 mb-3" style={serif}>
        ...
        <footer className="text-[11px] text-[#5A5751] mt-1">— Proverbs 4:7 (KJV). A curated, depth-adaptive Scripture resource that backs the spiritual teaching and the Learn lessons — explaining how Yahweh sees, what His heart is, and how His love speaks, held in grace and truth, all for the soul.</footer>
      </blockquote>

      {/* The governing lens */}
      <details className="mb-3 border border-[#E8E4DC] bg-white">
        <summary className="cursor-pointer px-3 py-2 text-[10px] uppercase tracking-[0.2em] text-[#B85838] font-semibold focus:outline focus:outline-2 focus:outline-[#B85838]">How this is taught — the lens</summary>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-3">
        <div className="border border-[#E8E4DC] bg-white p-2">
          <div className="text-[9px] uppercase tracking-wider text-[#5A5751] mb-1">Depth — read as deep as you need</div>
          <div className="flex gap-1" role="group" aria-label="Reading depth">
            {DEPTH_TIERS.map((d) => (
              <button key={d.id} type="button" onClick={() => setTier(d.id)} aria-pressed={tier === d.id} title={d.hint}
                className={`text-[10px] uppercase tracking-wider px-2 py-1.5 border flex-1 focus:outline focus:outline-2 focus:outline-[#B85838] ${tier === d.id ? 'bg-[#1A1815] text-white border-[#1A1815]' : 'bg-white text-[#5A5751] border-[#E8E4DC] hover:text-[#1A1815]'}`}>{d.label}</button>
            ))}
          </div>
        </div>
        <div className="border border-[#E8E4DC] bg-white p-2">
          <div className="text-[9px] uppercase tracking-wider text-[#5A5751] mb-1">Level — the same truth, for you</div>
          <div className="flex gap-1 flex-wrap" role="group" aria-label="Experience level">
            {EXPERIENCE_LEVELS.map((l) => (
              <button key={l.id} type="button" onClick={() => setLevel(l.id)} aria-pressed={level === l.id} title={l.hint}
                className={`text-[10px] uppercase tracking-wider px-2 py-1.5 border focus:outline focus:outline-2 focus:outline-[#B85838] ${level === l.id ? 'bg-[#1A1815] text-white border-[#1A1815]' : 'bg-white text-[#5A5751] border-[#E8E4DC] hover:text-[#1A1815]'}`}>{l.label}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      <details className="mb-3 border border-[#E8E4DC] bg-white">
        <summary className="cursor-pointer px-3 py-2 text-[10px] uppercase tracking-[0.2em] text-[#B85838] font-semibold focus:outline focus:outline-2 focus:outline-[#B85838]">Personalize what surfaces (optional)</summary>
        <div className="px-3 pb-3" style={serif}>
          ...
          <p className="text-[11px] text-[#5A5751] mb-2">{PRIVACY.consent} {PRIVACY.scope}</p>
          {consented && (
            <div className="flex flex-wrap gap-1.5">
              {INTEREST_TOPICS.map(([id, label]) => (
                <button key={id} type="button" onClick={() => toggleInterest(id)} aria-pressed={interests.includes(id)}
                  className={`text-[10px] uppercase tracking-wider px-2 py-1 border focus:outline focus:outline-2 focus:outline-[#B85838] ${interests.includes(id) ? 'bg-[#5A6E3D] text-white border-[#5A6E3D]' : 'bg-white text-[#5A5751] border-[#E8E4DC] hover:text-[#1A1815]'}`}>{label}</button>
              ))}
            </div>
          )}
          <p className="text-[10px] text-[#5A5751] mt-2 italic">{PRIVACY.youtube} Your viewing history feeds this same ordering when you connect a consented feed — until then, your chosen interests are the signal.</p>
        </div>
      </details>
      ...
      {results ? (
        <div>
          <p className="text-xs text-[#5A5751] mb-2" style={serif}>
            ...
            <button type="button" onClick={() => setQuery('')} className="ml-2 uppercase tracking-wider text-[10px] text-[#B85838] hover:text-[#1A1815]">Clear</button>
          </p>
          {results.length ? (
            <div className="space-y-2">
              {results.map((v) => (
