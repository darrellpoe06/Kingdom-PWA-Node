...
const btn = 'text-[0.625rem] uppercase tracking-wider px-3 py-1.5 min-h-[36px] border focus:outline focus:outline-2 focus:outline-[#B85838]';
  ...
  return (
    <div className="border border-[#E8E4DC] bg-white p-2.5 space-y-1.5">
      <div className="flex items-baseline justify-between gap-2 flex-wrap">
        <div className="text-[0.6875rem]" style={{ fontFamily: '"JetBrains Mono", monospace' }}>
        ...
        <span
          className="text-[0.5625rem] uppercase tracking-wider font-semibold px-2 py-0.5"
      ...
      {!readOnly && (
        <div className="flex gap-1.5 flex-wrap items-center">
          {a.phone ? (
            ...
            <span className="text-[0.625rem] text-[#B85838] italic" style={{ fontFamily: '"Fraunces", serif' }}>No phone on file — add it in Books · 1099s.</span>
          )}
          {!done && onWorkerDone && (
      ...
      {payoutOpen && !readOnly && (
        <div className="bg-[#FAF8F4] border border-[#E8E4DC] p-2 flex gap-2 items-end flex-wrap">
          <div>
            <label className="text-[0.5625rem] uppercase tracking-wider text-[#5A5751] block">Hours</label>
          ...
          <div>
            <label className="text-[0.5625rem] uppercase tracking-wider text-[#5A5751] block">Amount ($)</label>
...
export function DispatchPanel({
  ...
  if (resolved) {
    return (
      <div className="text-[0.6875rem] text-[#5A6E3D] font-semibold space-y-0.5" style={{ fontFamily: '"JetBrains Mono", monospace' }}>
    ...
    <div className="space-y-2">
      {assignments.length > 0 && (
        <div className="space-y-1.5">
          <div className="text-[0.5625rem] uppercase tracking-[0.25em] text-[#5A5751] font-semibold">
      ...
      {readOnly ? (
        assignments.length === 0 && (
          <div className="text-[0.6875rem] text-[#5A5751] italic" style={{ fontFamily: '"Fraunces", serif' }}>No worker assigned yet.</div>
        )
      ) : (
        <div className="space-y-1.5">
          ...
          {assignments.length > 0 && !adding ? (
            <button type="button" onClick={() => setAdding(true)} className="text-[0.625rem] uppercase tracking-wider text-[#B85838] hover:text-[#1A1815]">
            ...
            <>
              ...
              {contractors.filter(c => c.direction === 'outbound' && !DONE_STATUSES.has(c.status)).length === 0 ? (
                <div className="text-[0.6875rem] text-[#5A5751] italic" style={{ fontFamily: '"Fraunces", serif' }}>
