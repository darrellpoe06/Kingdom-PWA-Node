        ...
        <>
          ...
          <div className="bg-[#FAF8F4] border border-[#E8E4DC] p-2.5 mb-3">
            <div className="flex items-center justify-between gap-2 flex-wrap">
              <span className="text-[10px] uppercase tracking-wider text-[#1A1815] font-semibold">Conflict rate · {m.eventCount} events recorded</span>
              <KpiDot status={tv.status} label={tv.label} className="text-[9px] uppercase tracking-wider" />
            </div>
            {bars.length > 0 ? (
              <div className="flex items-end gap-2 mt-2 h-16">
                {bars.map((b) => (
                  <div key={b.bucket} className="flex flex-col items-center justify-end flex-1 min-w-[28px]">
                    <span className="text-[10px] text-[#1A1815] font-semibold mb-0.5">{b.count}</span>
                    <div
                      className="w-full"
                      style={{ height: `${Math.max(b.pct, 6)}%`, backgroundColor: '#B85838', minHeight: '3px' }}
                      aria-hidden="true"
                    />
                    <span className="text-[8px] text-[#5A5751] mt-1" style={{ fontFamily: '"JetBrains Mono", monospace' }}>{b.bucket.slice(5)}</span>
                  </div>
                ))}
              </div>
            ) : null}
          ...
          <ul className="border border-[#E8E4DC] mb-3">
            {m.hotFiles.map((h) => {
              ...
              return (
                <li key={h.file} className="px-2 py-1.5 border-b border-[#F2EEE6] last:border-b-0">
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <span className="text-[11px] text-[#1A1815]" style={{ fontFamily: '"JetBrains Mono", monospace' }}>
                      {h.file}{h.isMonolith ? ' ⟵ monolith' : ''}
                    </span>
                    <KpiDot status={hs.status} label={hs.label} className="text-[9px] uppercase tracking-wider shrink-0" />
                  </div>
                  {h.branches.length > 0 && (
                    <p className="text-[10px] text-[#5A5751] mt-0.5" style={{ fontFamily: '"Fraunces", serif' }}>
            ...
            {m.hotFiles.length === 0 && <li className="px-2 py-2 text-[11px] text-[#5A5751] italic">No hot files — every conflict was on a distinct file.</li>}
          </ul>
            ...
            <ul className="space-y-2">
              {m.decomposition.map((d) => (
                <li key={d.target} className="bg-[#FAF8F4] border border-[#E8E4DC] p-2.5">
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <span className="text-[11px] text-[#1A1815] font-semibold" style={{ fontFamily: '"JetBrains Mono", monospace' }}>{d.target}</span>
                    <span className="text-[9px] uppercase tracking-wider text-[#B85838] font-semibold shrink-0">P{d.priority} · {d.collisions}× collided</span>
                  </div>
                  <p className="text-[11px] text-[#5A5751] mt-1" style={{ fontFamily: '"Fraunces", serif' }}>{d.recommendation}</p>
                  {d.rankedExtractions.length > 0 && (
                    <ol className="list-decimal ml-4 mt-1 space-y-0.5">
                      {d.rankedExtractions.map((x, i) => (
                        <li key={i} className="text-[10px] text-[#5A5751]" style={{ fontFamily: '"Fraunces", serif' }}>{x}</li>
                      ))}
                    </ol>
                  )}
                </li>
          ...
          {m.problems.length > 0 && (
            <p className="text-[10px] text-[#B85838] mt-2">Spine problems: {m.problems.join('; ')}</p>
          )}
          <p className="text-[9px] text-[#5A5751] italic mt-3" style={{ fontFamily: '"Fraunces", serif' }}>
