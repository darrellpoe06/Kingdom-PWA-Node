// =============================================================================
// ClientGrowth — the in-app 3-sided "client growth" workflow surface (automated)
// =============================================================================
// The reusable 4-stage "revenue agent team" for TLC's THREE sides (Clients /
// Therapists / Training), now run as a SYSTEM rather than a manual job:
//
//   * ONE "Run the team" trigger chains all 4 stages end-to-end (each output
//     feeds the next), sovereign-A.I.-driven.
//   * The team AUTO-PRODUCES research, offers, content, sequences, draft leads;
//     leads land in the CRM and auto-advance across INTERNAL stages.
//   * APPROVE-OUTBOUND-ONLY: anything that goes OUT to a real person waits in the
//     approval queue for Christina/Darrell. Nothing auto-sends.
//   * Optional CONTINUOUS CADENCE — inert by default, behind the three brakes +
//     an explicit arm only Darrell sets.
//   * MARKETPLACE BALANCE — don't over-acquire one side.
//   * SUPPORTING LESSONS (lib/tlc-lessons) back each side as lead magnets + a
//     retention/outcomes engine.
//
// REALITY-TRACE: leads/funnel/balance come from the REAL synced practice_leads
// list (props). Stage drafts, runs, and the outbound queue are device-local
// working content (localStorage). The sovereign A.I. drafting runs on the NAS
// workflow wf-practice-growth; until it returns content the run produces the
// chained prompt-pack to capture by hand — never a fake A.I. result.
// =============================================================================
...
async function callPracticeGrowth(payload) {
...
function ClientGrowth({ leads = [], addLead, updateLead, deleteLead }) {
  ...
  // ========================================================================
  // RUN THE TEAM — one trigger chains all four stages end-to-end.
  // ========================================================================
  const runTheTeam = async () => {
    if (running) return; // single-flight in the UI
    setRunning(true);
    let run = newRun(config);
    setRuns((prev) => [run, ...prev.filter((r) => r.sideKey !== sideKey)]);
    ...
    for (const stage of ACQUISITION_STAGES) {
      ...
      let payload = null;
      try {
        payload = await callPracticeGrowth({ stage: stage.key, audience: sideKey, sensitivity: sensitivityFor(config), prompt });
        text = (payload && (payload.draft || payload.text || payload.output)) || '';
      } catch { /* unwired — handled below */ }

      if (text) {
        const out = newStageOutput(stage.key, text, { sideKey, runId: run.id });
        producedThisRun.push(out);
        addOutput(out);
        run = setRunStep(run, stage.key, { status: 'produced', outputId: out.id, message: '' });
      } else {
        ...
        producedThisRun.push(out);
        addOutput(out);
        run = setRunStep(run, stage.key, { status: 'needs-capture', outputId: out.id, message: 'Capture the A.I. output for this stage (NAS workflow pending).' });
      }
      setRuns((prev) => prev.map((r) => (r.id === run.id ? run : r)));

      // Auto-flow: if the NAS returned identified leads with the market-signal or
      // conversion stage, land them in the CRM and auto-advance across INTERNAL
      // stages only (never across the outbound boundary).
    ...
    run = { ...run, finishedAt: new Date().toISOString(), status: runOverallStatus(run), summary: { landedLeads, queuedOutbound } };
    setRuns((prev) => prev.map((r) => (r.id === run.id ? run : r)));
    setRunning(false);
  };
  ...
  const approveOutboundItem = (item) => {
    ...
    // Approving the outbound is what advances the lead across the outbound boundary.
  ...
  // Manual "draft outreach" for an outreach-ready lead (demonstrates the auto-flow
  // pre-NAS): pulls the latest approved sequence into a pending outbound item.
    ...
    <div className="space-y-5">
      ...
      <section className="bg-white border-2 border-[#1A1815] p-4 sm:p-5">
        <div className="flex items-center justify-between gap-3 flex-wrap mb-2">
          <div>
            <div className="text-[10px] uppercase tracking-[0.25em] text-[#B85838] font-semibold">Run the team · {SIDE_TAB_LABEL[sideKey]}</div>
            <p className="text-xs text-[#5A5751] mt-0.5" style={{ fontFamily: '"Fraunces", serif' }}>Chains all 4 stages in one run. Outputs land below; outbound waits for your approval.</p>
          </div>
          <button onClick={runTheTeam} disabled={running}
            ...
            {running ? 'Running the team…' : '⚡ Run the team'}
          </button>
        </div>
        {sideRun && (
          <div className="mt-2 border-t border-[#E8E4DC] pt-2 space-y-1">
            {sideRun.steps.map((s) => {
              const color = s.status === 'produced' ? 'text-[#5A6E3D]' : s.status === 'needs-capture' ? 'text-[#B85838]' : s.status === 'running' ? 'text-[#1A1815]' : 'text-[#5A5751]';
              const mark = s.status === 'produced' ? '✓' : s.status === 'needs-capture' ? '✎' : s.status === 'running' ? '…' : '·';
              return (
                <div key={s.stageKey} className="flex items-baseline justify-between gap-2 text-xs">
                  <span style={{ fontFamily: '"Fraunces", serif' }}>{s.emoji} {s.role}</span>
                  <span className={`uppercase tracking-wider text-[10px] ${color}`}>{mark} {s.status}</span>
                </div>
              );
            })}
            {sideRun.summary && (
              <p className="text-[11px] text-[#5A5751] italic pt-1" style={{ fontFamily: '"Fraunces", serif' }}>
                Landed {sideRun.summary.landedLeads} lead(s), queued {sideRun.summary.queuedOutbound} outbound draft(s) for approval.
              </p>
            )}
          </div>
        )}
