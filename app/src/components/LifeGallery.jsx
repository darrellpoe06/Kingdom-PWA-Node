...
// device-local record so it is safe from a phone change and rides sync.
// The large per-property photo archives (hundreds of images) reference the
// NAS in place instead — see docs/99-session-notes/2026-06-11-room-memory-
// and-image-ingest.md for that separate pipeline.
      ...
      <div className="flex items-baseline justify-between gap-3 mb-3 flex-wrap">
        <div>
          ...
          <p className="text-xs text-[#5A5751] italic mt-1" style={{ fontFamily: '"Fraunces", serif' }}>
            The people and work this is all for — kept safe from any phone change.
          </p>
        </div>
        {!readOnly && (
            ...
            {shown.map(p => (
              <figure key={p.id} className="border border-[#E8E4DC] bg-[#FAF8F4]">
                ...
                <figcaption className="p-2">
                  ...
                  {!readOnly && (
                    <button type="button" onClick={() => { if (deleteLifePhoto && window.confirm('Remove this photo?')) deleteLifePhoto(p.id); }} className="text-[9px] uppercase tracking-wider text-[#5A5751] hover:text-[#B85838] mt-1">× remove</button>
                  )}
                </figcaption>
              </figure>
            ))}
