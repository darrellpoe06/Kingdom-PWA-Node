  ...
  return (
    <div className="bg-[#FAF8F4] border border-[#1A1815] p-3 sm:p-4">
      <div className="flex items-baseline justify-between gap-2 mb-1">
        <h3 className="text-[10px] uppercase tracking-[0.25em] text-[#B85838] font-semibold">🛠 Conference setup</h3>
        <span className="text-[10px] text-[#5A5751]" style={{ fontFamily: '"JetBrains Mono", monospace' }}>{progress.done}/{progress.total} done</span>
      </div>
      ...
      <ul className="space-y-1.5">
        {steps.map((s) => (
          <li key={s.key} className="flex items-start gap-2">
            <span className="shrink-0 mt-0.5"><KpiDot status={TONE[s.status]} label="" className="text-[10px]" /></span>
            <span className="min-w-0 flex-1">
              <span className="text-[11px] font-semibold text-[#1A1815]" style={{ fontFamily: '"Fraunces", serif' }}>
                <span aria-hidden="true">{MARK[s.status]}</span> {s.title}
              </span>
              <span className="block text-[11px] text-[#5A5751]" style={{ fontFamily: '"Fraunces", serif' }}>{s.value}</span>
              {s.hint && <span className="block text-[10px] text-[#8A6E1F] italic" style={{ fontFamily: '"Fraunces", serif' }}>→ {s.hint}</span>}
            </span>
          </li>
        ))}
      </ul>
