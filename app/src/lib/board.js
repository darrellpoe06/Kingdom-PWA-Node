...
export const SEED_BOARDS = [
  ...
  // loop hitting multiple users, and the unified admin/user front door).
  {
    ...
    items: [
      ...
      { key: 'cf-runbook', group: 'Provisioning', title: 'Per-client provisioning runbook (instance + seats + registry row)', status: 'not-started', owner: 'Ari', notes: 'The repeatable checklist from CLIENT-BUSINESS-FACTORY.md steps 4-6, executed per client; target clock: same day.' },
      ...
      { key: 'cf-voice-discovery', group: 'Commercial', title: 'Recorded discovery — voice notes / LLM conversation → reviewed requirements → MVP', status: 'in-progress', owner: 'Ari', notes: 'Parser (parseDiscoveryJson, source_quote on every item, status=extracted until steward review) + the extraction contract (infra/nas-sme-pipeline/client-discovery-json-prompt.md) SHIPPED. Next: the NAS transcription ride (existing Whisper rails) + the in-app review/import surface.' },

      // ── Discovery validation (Moore) — the Current → Future → Gap → Decision
      //    lane (DR-0118; Darrell's Mosaic-board workflow applied to the first
