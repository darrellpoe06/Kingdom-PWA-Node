...
// seedDueDate — the PLANNED target date a SEED item carries in its spec (Ari's
// PM forecast, sequenced by each item's stated dependencies). Used to gap-fill
// an ALREADY-LOADED row that has no dueDate, so the 12-Month Workload Forecast
// shows the plan instead of "N items undated" (Darrell 2026-07-11: "timelines
// haven't and aren't filled why"). Gap-fill ONLY — the heal never overwrites a
// date the family set (their real date is always senior to the forecast).
// Returns undefined for a non-seed / unknown slug or an item with no target.
export function seedDueDate(boardSlug, slug) {
  const spec = SEED_BOARD_BY_SLUG[boardSlug];
  if (!spec) return undefined;
  const prefix = `bt-seed-${boardSlug}-`;
  if (!slug || !slug.startsWith(prefix)) return undefined;
  const key = slug.slice(prefix.length);
  const item = (spec.items || []).find((it) => it.key === key);
  return item && item.dueDate ? item.dueDate : undefined;
}
...
export const SEED_BOARDS = [
  // ── 1. Modular cutover — the monolith → modules decomposition ─────────────
  {
    ...
    items: [
      ...
      { key: 's2', group: 'Stages', title: 'Stage 2 — migrate existing surfaces onto the registry-driven mount', status: 'in-progress', dueDate: '2026-08-15', owner: 'Ari', notes: 'Registry holds every lazy loader; the shell still uses named exports + render switch. Replace the switch with a registry-driven mount (derive nav C3/C4, route allow-list C5, feedback-area C6).' },
      { key: 's3', group: 'Stages', title: 'Stage 3 — peel remaining in-file sections (Church wrapper, BigPicture, Admin, FeedbackModal)', status: 'not-started', dueDate: '2026-09-30', owner: 'Ari', notes: 'Work queue = ranked hot-file output (conflict-analytics.mjs), highest-contention first. One section per PR, behavior pinned by tests.' },
      { key: 's4', group: 'Stages', title: 'Stage 4 — boundary gate + timestamped migration names', status: 'in-progress', dueDate: '2026-08-31', owner: 'Ari', notes: 'module-boundary-guard.mjs + monolith-budget-guard.mjs exist. Remaining: timestamped migration allocator (YYYYMMDDHHMM-slug.sql) to end the duplicate-number class.' },
      { key: 's5', group: 'Stages', title: 'Stage 5 — full decomposition (monolith becomes a thin composition root)', status: 'not-started', dueDate: '2026-11-30', owner: 'Ari', notes: 'After the in-flight queue drains + soak confirms the conflict trend turned DOWN.' },
    ],
  },
  ...
  // ── 2. Church LED wall + infrastructure ───────────────────────────────────
  {
    ...
    items: [
      ...
      { key: 'led-finish', group: 'LED video wall', title: 'Punch list — warranty modules + EDID 2560x1440 + NovaLCT backup to NAS', status: 'in-progress', dueDate: '2026-07-31', owner: 'Darrell', notes: 'A few dark modules (vendor warranty swap, positions photographed); input EDID nicety for 1:1 pixels; copy the NovaLCT config export to the NAS share.' },
      ...
      { key: 'inf-nas', group: 'Compute + AV stack', title: 'Church NAS — the brain + barn (storage, services, backup)', status: 'in-progress', dueDate: '2026-08-31', owner: 'Darrell', notes: 'Sovereign church stack. 3-2-1 backup incl. encrypted sealed-blob offsite. Ties infrastructure-class.js.' },
      { key: 'inf-towers', group: 'Compute + AV stack', title: 'Two compute towers (RTX 4070 pair) — Claude residents + Python pipelines', status: 'in-progress', dueDate: '2026-08-15', owner: 'Darrell', notes: 'Tower 1 VERIFIED 2026-07-03: livestream-main-pc, RTX 4070 12GB, driver 595.95, Claude Code resident (Claude Max), faster-whisper install in flight, RustDesk-over-Tailscale chosen (Win11 Home = no native RDP host). Tower 2: repeat the recipe (~1 hr); it runs Proclaim + ATEM control Sundays, AI batch in the quiet window.' },
      ...
      { key: 'inf-walkthrough', group: 'Network project', title: 'Walk-through inventory — rack/closet photos, switches, APs, ISP/modem', status: 'not-started', dueDate: '2026-09-15', owner: 'Darrell', notes: 'Photos of the rack/closet + switch make/models feed the device register; the network-map command on each machine fills the subnet/ARP picture.' },
      { key: 'inf-rmm', group: 'Network project', title: 'Identify the Tactical RMM agent operator (found on tower 1)', status: 'not-started', dueDate: '2026-09-30', owner: 'Darrell', notes: 'Third-party remote-management agent on livestream-main-pc (observed updating Tailscale via winget). Who operates it? Security-posture answer required before the VLAN design.' },
      { key: 'inf-network', group: 'Network project', title: 'UniFi gateway + VLAN walls (family · COLG · TLC · Properties · PoeTech)', status: 'not-started', dueDate: '2026-10-31', owner: 'Darrell', notes: 'One door to the internet; separate walled VLAN rooms per domain. Designed AFTER the walk-through inventory.' },
    ],
  },
  ...
  // to the PoeTech App build so we can run these projects as a team."
  {
    ...
    items: [
      { key: 'med-docx', group: 'Ingest (Wednesday)', title: 'Index the local Proclaim docx archive (tower 2)', status: 'not-started', dueDate: '2026-08-15', owner: 'Darrell', notes: 'proclaim_docx_index.py — stdlib-only; reads the docx folder the Proclaim team already keeps; extracts date/title/scripture/preacher + segments into proclaim-index.json. One instruction to tower 2\'s Claude runs it.' },
      { key: 'med-gmail', group: 'Ingest (Wednesday)', title: 'Gmail ingest — BG\'s weekly PROCLAIM email + .docx attachments (NAS Python)', status: 'not-started', dueDate: '2026-08-31', owner: 'Ari', notes: 'gmail_ingest.py per DR-0083: full-history pagination (200+ threads verified in the mailbox 2026-07-03), attachment scope, three brakes, run-state to the Loops surface. The one human piece is the Gmail OAuth credential only Darrell holds.' },
      { key: 'med-load', group: 'Ingest (Wednesday)', title: 'Load the PROCLAIM index into The Word (choir_sermons + sermon_prep)', status: 'not-started', dueDate: '2026-08-31', owner: 'Ari', notes: 'proclaim_load.py — the MISSING step (2026-07-14): the ingest pulled + indexed the .docx but nothing wrote it to the DB The Word reads, so the library sat empty. Structures the paragraphs into numbered points + scriptures and upserts choir_sermons + sermon_prep as GROUND TRUTH (source=email, needs_review=false). Dry-run default, idempotent, service-key REST, ships inert. Parse layer unit-tested (test_proclaim_load.py). Runs after the docx index; the public points RPC (0101) then shows the points to everyone.' },
      { key: 'med-oos', group: 'Ingest (Wednesday)', title: 'Order of Service tab — wire to the real BG format', status: 'not-started', dueDate: '2026-09-15', owner: 'Ari', notes: 'The tab exists and waits (church-program). The first indexed docx defines the standard-order template; the ingest keeps it fresh weekly.' },
      { key: 'med-precheck', group: 'Go-live (Sunday)', title: 'Pre-service check — GO/NO-GO an hour before service', status: 'not-started', dueDate: '2026-09-30', owner: 'Ari', notes: 'preservice_check.py on booth/tower: disk, GPU, tailnet peers, targets reachable. Problems surface at 9am, not 10:05. A human runs it; the script only reports.' },
      ...
      { key: 'med-atem', group: 'Go-live (Sunday)', title: 'ATEM CLI tools — scripted switcher controls, weekday-tested, human-pressed', status: 'not-started', dueDate: '2026-12-15', owner: 'Ari', notes: 'The ATEM speaks a network protocol with solid libraries; build cut/macro/stream CLI per AI-FOUNDATION (a click today = an API call tomorrow). ASSISTIVE only — never autonomous during live service (three brakes; human hands on Sunday).' },
      { key: 'med-youtube', group: 'Harvest (after service)', title: 'YouTube channel index → whisper queue (the videos ARE the camera switches)', status: 'not-started', dueDate: '2026-10-15', owner: 'Ari', notes: 'youtube_index.py lists the channel uploads into the tower whisper queue. The published program output is also the switching ground truth for future assistive-ATEM training.' },
      { key: 'med-whisper', group: 'Harvest (after service)', title: 'faster-whisper transcription on the towers — no video lost', status: 'in-progress', dueDate: '2026-07-31', owner: 'Darrell', notes: 'Pipeline install started on tower 1 (2026-07-03): Python 3.12 + ffmpeg + faster-whisper (no PyTorch needed — CTranslate2 wheel). Paused at an approval prompt; resumes on next touch. Then the back-catalog becomes a batch job.' },
      { key: 'med-library', group: 'Harvest (after service)', title: 'One linked record per sermon: video + transcript + points + order', status: 'not-started', dueDate: '2026-10-31', owner: 'Ari', notes: 'The rails exist (sermon-points, sermon-library-sync, harvest-ledger). This task is the join once ingest + harvest both land.' },
    ],
  },
  ...
  // ── 3. Financial loops ────────────────────────────────────────────────────
  {
    ...
    groupOrder: ['Pipeline'],
    items: [
      { key: 'fin-import', group: 'Pipeline', title: 'Bank statement import (email → Gmail → NAS → verified ledger)', status: 'in-progress', dueDate: '2026-07-31', owner: 'Ari', notes: 'System work (wf18 import, repaired 2026-06-17). Coverage loaded Nov24–May26 (4 Chase accts, 1932 txns). Ari runs the import; the one human sub-step (Gmail reconnect — a credential Darrell holds) is pushed to Darrell when it comes due.' },
      { key: 'fin-categorize', group: 'Pipeline', title: 'Categorize + verify imported transactions', status: 'in-progress', dueDate: '2026-08-15', owner: 'Ari', notes: 'Ari categorizes each txn; the human spot-verify before a txn is load-bearing is a push-to-Darrell checkpoint, not the owner of the whole loop. Books figures derive from the ledger (account-balances.js), never a stored balance.' },
      ...
      { key: 'fin-debts', group: 'Pipeline', title: 'Debts — snowball/avalanche payoff tracking', status: 'in-progress', dueDate: '2026-08-31', owner: 'Ari', notes: 'System computes snowball/avalanche payoff order + progress from the ledger. Debts flow into the forecast as obligations. Darrell governs the strategy choice; the tracking is Ari.' },
  ...
  // ── 4. TLC training library ───────────────────────────────────────────────
  {
    ...
    items: [
      { key: 'tlc-courses', group: 'Courses + CE', title: 'Clinician courses — 10 fields, pre/post tests, hours + cert', status: 'in-progress', dueDate: '2026-09-30', owner: 'Christina', notes: 'FOUR-STRAND spine (Yahweh centre + clinical + neuroscience + societal); multi-track grounded IL/CSWE hours. PracticeLearn.jsx, PR #429.' },
      { key: 'tlc-ceu', group: 'Courses + CE', title: 'CE/CEU renewal tracker (multi-state)', status: 'in-progress', dueDate: '2026-10-15', owner: 'Christina', notes: 'ceu-tracker.js STATE_RULESETS; IL ruleset confirmed:false pending Christina SME. 0055.' },
      { key: 'tlc-sme', group: 'Courses + CE', title: 'Christina SME gate — clinical content review before publish', status: 'not-started', dueDate: '2026-10-31', owner: 'Christina', notes: 'SME sign-off gate on clinician-facing content. Reviewed by use in her hands.' },
      { key: 'tlc-ingest', group: 'Courses + CE', title: 'YouTube-distill ingest (Rivah/Ellis → Couples track)', status: 'not-started', dueDate: '2026-11-30', owner: 'Ari', notes: 'Distill SME video → course modules; local NAS pipeline.' },
    ],
  },
  ...
  // discovery is the only shipped piece at kickoff.
  {
    ...
    items: [
      ...
      { key: 'md-dns', group: 'Front door', title: 'Custom domain DNS (mooredivahs)', status: 'not-started', dueDate: '2026-08-15', owner: 'Darrell', notes: 'Real-world step only Darrell can do: buy the domain + point DNS at the Cloudflare Pages project. Not blocking — poetech.us/moore is the her-name link until then.' },
      { key: 'md-handles', group: 'Front door', title: 'Shay\'s real social handles (IG / FB / TikTok)', status: 'not-started', dueDate: '2026-07-31', owner: 'Darrell', notes: 'Values only Shay holds; wire into the intake link + follow-ups. Email confirmed: mooredivahs1@yahoo.com.' },
    ],
  },
  ...
  // loop hitting multiple users, and the unified admin/user front door).
  {
    ...
    items: [
      ...
      { key: 'cf-registry', group: 'Registry', title: 'Business registry — one generic door at ?biz=<slug>', status: 'done', owner: 'Ari', notes: 'SHIPPED (held through demo day per DR-0075, executed on the 07-09 re-review): BUSINESS_REGISTRY rows (lib/business-registry.js) + BusinessDoor at ?biz=<slug>; Moore = row #1, ?moore=1 permanent alias; door engine reads brand/manifest/tabs/instance-slugs/capture attribution from the row; unknown slug = honest no-business card. PROVEN in-browser: alias and ?biz render identical doors. Per-section interior config intentionally rides client #2\'s REAL needs (no speculative flags).' },
      ...
      { key: 'cf-relay', group: 'Provisioning', title: 'Inquiry email relay per business (owner notified, address never public)', status: 'not-started', dueDate: '2026-09-30', owner: 'Darrell', notes: 'NAS-side sender (three brakes); the credential is Darrell\'s hand. Until then each owner\'s inbox is their CRM pipeline view.' },
      ...
      { key: 'cfv-all-decision', group: 'Discovery validation (Moore)', title: 'One generic registry-driven door (?biz=slug) — Moore converts to the first row', status: 'not-started', dueDate: '2026-08-15', owner: 'Ari', notes: 'cf-registry: a new client door = a registry row, never a new component. Until it lands, the outcome here stays honestly unknown.', links: { flow: 'decision', unit: 'All units', outcome: 'unknown' } },
