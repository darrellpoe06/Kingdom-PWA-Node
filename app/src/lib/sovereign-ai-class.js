...
export const SOVEREIGN_AI_MODULES = [
  ...
  //
  // REBUILT 2026-08-25 at Darrell's correction ("I gave more information in
  // the prompt than that whole lesson... Word first?!... rigorous lessons for
  // competence... big and little macro and micro"): the deep lesson now runs
  // as TEN numbered movements that open with the Word (Genesis 1's own
  // inspection loop) and carry the WHOLE article — every section, every
  // filter, the three loops, the risk dial — plus the primary-source research
  // (DORA 2024+2025, METR, Veracode, GitClear, the 100x provenance) woven in
  // where each claim is taught, not parked in a postscript. Every quoted
  // verse is verbatim KJV, pinned in sovereign-ai-verse-integrity.test.js.
  {
    id: 'sov9-verification-earns-trust',
    title: 'Why verification matters in the age of A.I. — trust is earned, never claimed',
    bigIdea: 'Verification is the Maker\'s own pattern before it is an industry practice: six times in Genesis 1 Yahweh inspected His work in progress, then reviewed the whole and called it very good — and "Prove all things" is the command form. The industry has just rediscovered it: A.I. made WRITING code cheap, so VERIFYING code is now the precious work, and trust must be EARNED in degrees — filters, tests, independent witnesses, live watching — never granted to confidence. Our house wrote it as law before the article arrived (DR-0076): not sounding right — VERIFIABLY right.',
    ...
    anchor: { ref: '1 Thessalonians 5:21; Proverbs 14:15', theme: 'KJV: "Prove all things; hold fast that which is good." And: "The simple believeth every word: but the prudent man looketh well to his going." Verification is not distrust of people — it is the prudence the Word commands. The full spine of this week: Genesis 1:31; Job 12:11; Romans 3:4; Jeremiah 17:9-10; Luke 14:28-30; Proverbs 22:3; Deuteronomy 19:15; 2 Corinthians 13:1; John 5:31; Proverbs 18:17; 1 John 4:1.'  },
    rpe: {
    ...
    levels: {
      teen: 'Before any software company said it, Yahweh showed it: six times in Genesis 1 He inspected His own work in progress, and at the end He reviewed everything He had made and called it very good. Building and CHECKING belong together — that is the Maker\'s pattern. Now the industry has rediscovered it: A.I. writes working code in seconds, so the hard, valuable work moved from WRITING to VERIFYING — earning enough trust to put code in front of real people. Trust is earned like a contract: writing the words is easy; review and signatures make it real. Teams earn it with a stack of filters — instant automatic checks, then tests against known answers (a function meant to ADD that secretly MULTIPLIES passes every style check and fails the test), then a real person judging fit, then live monitoring underneath. The measured surprises: developers using A.I. FELT about 20% faster and MEASURED 19% slower (the heart is deceitful — Jeremiah 17:9 — so we trust measurement over feeling); A.I. code carried a known security flaw about 45% of the time, and models got far better at making code RUN without getting better at making it SAFE. And the deepest catch: an A.I. reviewing an A.I. built like itself shares its blind spots — one opinion stated twice. The Word made real verification a law of witnesses long ago: two or three INDEPENDENT witnesses establish a matter (Deuteronomy 19:15) — even Jesus said "If I bear witness of myself, my witness is not true" (John 5:31). Our app lives all of this: 8,000+ tests and hard gates on every change, and a red gate blocks anything, no matter how confident it sounded. "Prove all things; hold fast that which is good."',
      senior: 'The Word first: Genesis 1 is a build log with an inspection step — six in-progress reviews and a final whole-system review (1:31) — and the doctrine of this week is that pattern applied to software. The industry data, verified at the primary sources: DORA 2024 (read) — a 25% rise in A.I. adoption associated with a 7.2% drop in delivery stability, 1.5% throughput dip, 39.2% reporting little or no trust in A.I. output; DORA 2025 (~5,000 respondents) — throughput flipped positive while instability persists: faster AND less stable at once. METR (read) — 16 experienced developers, 246 real issues in their own mature repos, randomized: 19% SLOWER with A.I. against a 24% forecast speedup, while still believing they were ~20% faster; METR itself calls the figure an early-2025 snapshot. Veracode 2025 (corroborated accounts; primary PDF unreachable from the session, flagged honestly) — 80 tasks across 100+ models: 45% of completions carried an OWASP Top-10 flaw, XSS failed 86% and log injection 88%, syntax pass ~50%→~95% since 2023 while security stayed flat at 45-55%. GitClear (primary PDF read) — 211M changed lines 2020-2024: duplicated-block commits 0.45%→6.66%, moved-line reuse 24.8%→9.5%, 2024 the first year copy/paste outnumbered moves. The mechanics, in full: trust earned in degrees through the filter stack (type checks, linters, unit tests, human review, monitoring, plus security and dependency scanners), split into static analysis (fast, broad, runtime-blind) and dynamic analysis (real behavior, exercised paths only); every filter tuned against the false-positive/false-negative tension, where alarm fatigue erodes a tool\'s authority until it is switched off (Malagodi\'s CAP-like trade: speed, accuracy, coverage — raise what a developer can act on); the pipeline pricing the same defect higher at every later stage — shift left, and Sonar\'s "starting left": a secrets guard at the terminal before the paste, because most leaks are a config pasted into an A.I. session. The 100x-cost multiplier itself traces to IBM training notes, not a documented study — direction true, precision folklore. The A.I.-specific pressures: volume and batch size (the 5,000-line "looks good to me"), the security gap, and the independence problem — a reviewer trained like the writer is one opinion stated twice, which the Word codified as insufficient at Deuteronomy 19:15 / 2 Corinthians 13:1, Jesus affirming it of Himself at John 5:31, Proverbs 18:17 sketching adversarial review, 1 John 4:1 commanding the spirits be TRIED. The mature 2026 stack: shared context fed up front against brownfield "box of chocolates" variance; three loops (agentic, CI verification, maintenance/remediation — tangled code measurably costs more tokens over time); explainable review findings; and a risk DIAL — typo versus payment bug, a comma away from a crashed OS — with agents merging low-risk work and routing high-risk to humans. Our house encoded the whole doctrine as DR-0076 before the article existed, and this platform is the live lab: proven-to-catch gates, adversarial live RLS proofs as true independent witnesses, the OpsBoard as the visible verify-loop. "Prove all things" is the engineering standard because it was the Kingdom standard first.',
    },
    quiz: {
      questions: [
        ...
        { q: 'The Word\'s two-or-three-witnesses law (Deuteronomy 19:15; 2 Corinthians 13:1) maps to which verification truth?', options: ['Any two tools make code trustworthy', 'The witnesses must be INDEPENDENT — a reviewer trained like the writer is one opinion stated twice, not a second witness', 'More reviewers always mean more truth'], answer: 1, explain: 'Even Jesus said "If I bear witness of myself, my witness is not true" (John 5:31). Independence is what makes a second check establish anything — which is why our high-stakes proofs attack the live system instead of re-reading the code.' },
        { q: 'METR\'s developers believed they were ~20% faster with A.I.; the measurement said 19% slower. What does the house do with that gap?', options: ['Trust the experienced developers\' feel', 'Measure, don\'t claim — "The heart is deceitful above all things" (Jeremiah 17:9), so the lane trusts measurements and gates over anyone\'s confidence', 'Stop using A.I. entirely'], answer: 1, explain: 'The feeling and the measurement disagreed, and the feeling lost. That is DR-0076\'s "measure, don\'t claim" — Jeremiah 17:9-10 applied to a build lane: Yahweh Himself tries the reins rather than trusting the heart\'s report.' },
      ],
    },
    lesson: 'FIRST, THE WORD SET THE STANDARD BEFORE THE INDUSTRY FOUND IT. Verification is not a software invention — it is the Maker\'s own pattern on page one of the Word. Six times in Genesis 1 Yahweh pauses His own build to inspect the work in progress (verses 4, 10, 12, 18, 21, 25), and when the whole work stands assembled He reviews it end to end: "And God saw every thing that he had made, and, behold, it was very good." (Genesis 1:31, KJV). The Creator does not need the inspection; He wrote the inspection into the record so builders would copy the loop — build, examine, and only then proceed. The command form is 1 Thessalonians 5:21: "Prove all things; hold fast that which is good." The temperament is Proverbs 14:15: "The simple believeth every word: but the prudent man looketh well to his going." Job names it as designed sense: "Doth not the ear try words? and the mouth taste his meat?" (Job 12:11). And the ground under all of it is Romans 3:4 — "let God be true, but every man a liar" — every claim by man or machine stands under a standard of truth higher than its own confidence. Our house wrote this into law as DR-0076 on 2026-06-15, before the industry article this week studies arrived: the A.I.\'s job is not to sound right — it is to be VERIFIABLY right, or to clearly mark what is unverified. Hold that frame; everything the industry below has just discovered, the Word already held. SECOND, THE SHIFT — WRITING BECAME CHEAP, SO PROVING BECAME PRECIOUS. For decades writing code was the slow, expensive step and review was a small task at the end. A.I. flipped the balance: a working function in seconds, a full feature in minutes, more machine-written code every month — so the scarce work moved from producing a change to deciding whether it belongs in production, and more code written simply means more code to verify. The house checked the industry\'s evidence at its PRIMARY sources, per DR-0076 — never cite unread. Google\'s DORA program (2024 report, read): a 25% increase in A.I. adoption was associated with a 7.2% DECREASE in delivery stability and a 1.5% throughput dip, and 39.2% of respondents reported little or no trust in A.I.-generated code. DORA\'s 2025 follow-up (~5,000 respondents) adds the fuller truth: throughput has since flipped POSITIVE while instability persists — teams now move faster AND break more, both at once. Speed bought at the keyboard is being paid for down the line, exactly where verification lives. THIRD, THE HEART THAT FELT FASTER — AND MEASURED SLOWER. The METR controlled trial (the study itself, read): 16 experienced open-source developers working 246 real issues in their own mature repositories, each task randomly assigned to allow or forbid A.I. tools. The developers forecast A.I. would speed them up by roughly a quarter; A.I.-allowed tasks instead ran 19% LONGER than the 24% forecast speedup — the time went into prompting, waiting, reading, and correcting the output — and afterward the developers still believed they had been about 20% faster. METR\'s own follow-up treats the figure as an early-2025 snapshot, not a law, and that honesty is carried here. But the deep finding is older than software: the FEELING of productivity and the MEASUREMENT of it disagreed, and the feeling lost. "The heart is deceitful above all things, and desperately wicked: who can know it?" (Jeremiah 17:9, KJV) — and the answer stands in the next verse: "I the LORD search the heart, I try the reins" (Jeremiah 17:10). Yahweh Himself verifies rather than trusting the feeling. Measure, don\'t claim — DR-0076\'s fourth point — is that verse applied to a build lane. FOURTH, TRUST IS EARNED IN DEGREES — COUNT THE COST BEFORE THE TOWER. Code verification is the umbrella for every check that a change is correct, safe, and maintainable enough to ship: the work of EARNING enough trust to put it in front of real people. The key word is earning — trust arrives in degrees, one check at a time, never granted in a single stroke. Like a contract: drafting the words is one part of the task, but the review, the legal checks, and the signatures are what turn words into something a person can rely on. The moment code leaves an editor and lands in the repository it carries an implicit claim about what it does, and verification is that claim being tested until the team can stand behind it. Life-critical domains push this to its limit with rigorous FORMAL verification — mathematically proving the code matches a precise specification, standard for flight-control systems and kernels where one defect risks lives; for most software that costs more than it returns, so teams layer lighter checks instead. Jesus taught the principle as plain prudence: "For which of you, intending to build a tower, sitteth not down first, and counteth the cost, whether he have sufficient to finish it?" (Luke 14:28, KJV) — and He named the price of skipping it: the unfinished tower, publicly mocked (Luke 14:29-30). Production is where unverified work gets mocked. FIFTH, THE FILTER STACK — EVERY LAYER CATCHES WHAT THE ONE ABOVE IT CANNOT. The cheapest checks sit on top. A TYPE CHECKER confirms every value moving through the code is the exact type each operation expects, killing a whole class of mistakes before the code even runs. A LINTER scans for suspicious patterns and style problems in an instant, at almost no cost. Below them sit TESTS: a unit test runs a small piece of code against known inputs and confirms the expected output — behavior the type layer cannot see, because a function meant to ADD two numbers that instead MULTIPLIES them is perfectly typed, perfectly lint-clean, and wrong; only the test comparing against a known answer catches it. Below tests, HUMAN REVIEW: another builder judges what machines miss — does this FIT the system, does it solve the RIGHT problem, is it readable, does it honor the house\'s standards. Beneath everything, PRODUCTION MONITORING watches the code under real traffic for whatever passed every earlier layer — and real stacks add more layers still: security scanners, dependency checks. The filters fall into two families. STATIC analysis reads the source without running it — fast and broad, a whole codebase in one pass, but partly blind to runtime, so it can raise alarms about problems live conditions never produce. DYNAMIC analysis runs the code with real inputs and observes — true behavior, but only on the paths actually exercised: a suite that only runs the happy path cannot see the crash waiting on an empty input. A clean scan and a green suite together can STILL leave gaps — which is why verification is many filters working together, never one hero check. SIXTH, FALSE ALARMS — SIGNAL QUALITY IS AS PRECIOUS AS COVERAGE. Every filter can fail two ways: a FALSE POSITIVE flags a problem that is not there; a FALSE NEGATIVE stays silent while a real bug slips through. Tune a tool to catch everything and it floods builders with false alarms; tune it to speak only when certain and it starts missing real defects — the two pull against each other, and the cost of crying wolf is steep and documented: static-analysis research shows high false-positive rates erode trust until teams switch the tool off or stop reading its warnings, and then even the occasional REAL warning gets waved away with the noise — reopening the door to the very bugs the tool existed to stop. Sonar\'s CTO Andrea Malagodi frames the balance as almost a CAP theorem for verification: SPEED, ACCURACY, and COVERAGE pull against each other and no tool fully wins all three, so his team\'s rule keeps the focus on people — a finding a developer can ACT on is worth raising. DR-0076 says the same from the other side: a gate that always passes is itself a lie — and a gate that always screams becomes one. SEVENTH, THE PIPELINE — SHIFT LEFT, AND START EVEN FURTHER LEFT. A change moves from the editor, to automated checks the moment it is committed, to review, to merge, to deployment, to live monitoring — and the SAME check grows more expensive the later it runs, because more work piles on top of the mistake: a flaw caught in the editor costs a moment of the author\'s attention; the same flaw in production costs an incident, a rollback, and hurt users. That is the real meaning of "shift left": move checks earlier, where mistakes are cheap. The article itself warns that vendors dress this truth in precise multipliers, and the house\'s research proved the warning right: the famous chart claiming a bug costs 100x more in production traces to an IBM internal training unit\'s course notes — no documented empirical study behind the numbers (the provenance investigation the article linked, read directly). The DIRECTION — later is costlier — stands on separate, weaker, context-dependent evidence; the multipliers are folklore, and this course refuses to teach folklore as measurement. Wisdom already held the true principle without the fake precision: "A prudent man foreseeth the evil, and hideth himself: but the simple pass on, and are punished." (Proverbs 22:3, KJV). Sonar pushes one step earlier than shift-left and calls it STARTING LEFT: a secrets scanner at the terminal itself, because most leaked credentials are not sabotage but a paste — someone loads code or a config file into an A.I. session, the key rides along, and it is caught too late, once it already lives in a commit or a log. Our house already walks that line as law: keys never pass through chat; they are placed at the machine, silently, or not at all. EIGHTH, THE INDEPENDENCE LAW — ONE OPINION STATED TWICE IS NOT TWO WITNESSES. With more code than people can carefully read, A.I. review of A.I. code has real momentum and real advantages: SPEED — it scans a change the moment it appears; COVERAGE — it catches a meaningful share of bugs and security issues early; CONSISTENCY — the same standards on every change and every team member, though A.I. review is probabilistic by nature, which is why it must be layered with deterministic, algorithmic tools rather than trusted alone. It can even run inside the agent\'s own loop — the agent drafts, the reviewer flags, the agent corrects — before a person ever looks, clearing routine work off human plates. But here is the catch, and it is the deepest warning in the whole subject: a reviewer model built from the same kind of training as the writer works from the same assumptions, and therefore shares the same BLIND SPOTS. It can confirm the code LOOKS right while the question that matters — does it do what was actually INTENDED — goes unasked, because pattern-matching cannot answer intent. Consider an agent that turns a ticket into a function and an A.I. reviewer that reports it clean: it compiles, runs, and matches common patterns, yet whether it does what the ticket truly MEANT cannot be answered by resemblance. Two similar models are one opinion stated twice, not two independent checks. The Word made independence a LEGAL requirement thousands of years ago: "at the mouth of two witnesses, or at the mouth of three witnesses, shall the matter be established" (Deuteronomy 19:15, KJV), carried into the church verbatim: "In the mouth of two or three witnesses shall every word be established." (2 Corinthians 13:1). Jesus submitted Himself to that standard: "If I bear witness of myself, my witness is not true." (John 5:31) — self-attestation does not establish truth, even for the Son of Yahweh; how much less for a model grading its sibling. Proverbs described adversarial review three thousand years early: "He that is first in his own cause seemeth just; but his neighbour cometh and searcheth him." (Proverbs 18:17). And the Word commands testing the SOURCE, not admiring the output: "Beloved, believe not every spirit, but try the spirits whether they are of God" (1 John 4:1). The industry\'s two camps — models can shrink or remove human review, versus humans stay essential for architecture, context, and accountability — resolve exactly as the article concedes: it depends on what you are shipping and what a failure would cost. Our house\'s answer is built, not argued: the high-stakes walls are proven by ATTACKING them on the live system — an independent witness by construction, never a re-read by a sibling. NINTH, THE MEASURED DAMAGE — SAFETY FLAT, DUPLICATION RISING. The volume pressure is real: an agent writes a thousand lines in the time a person once wrote a hundred, and A.I. tools tend to produce LARGER changes — attention spreads thin across a big diff until the 5,000-line pull request gets a weary "looks good to me" and production is left to find the mistakes (Andrea names that exact failure). The KIND of mistake matters more. Veracode\'s 2025 GenAI Code Security Report — 80 curated tasks across 100+ models (figures corroborated across multiple independent accounts; the primary PDF was unreachable from this session and this note says so, per DR-0076 §8) — found 45% of completions introduced an OWASP Top-10 vulnerability; cross-site-scripting defenses failed in 86% of cases and log-injection defenses in 88%; and while syntax pass rates climbed from ~50% to ~95% since 2023, security pass rates stayed FLAT between 45% and 55%. Sharply better at RUNNING, barely better at SAFE — and the gap widening. The maintainability drift is measured too. GitClear\'s research (the primary PDF, read): 211 million changed lines, 2020-2024; commits containing a duplicated block rose from 0.45% (2022) to 6.66% (2024) — roughly ten times in two years; MOVED lines, the signature of real reuse and refactoring, fell from 24.8% to 9.5%; and 2024 was the first measured year copy/pasted lines outnumbered moved ones. More code, more copies, less craft — the opposite of work done heartily as to the Lord (Colossians 3:23), and a debt some future builder pays. TENTH, THE MODERN STACK, THE RISK DIAL, AND THE HOUSE\'S OWN LANE. Most engineering is BROWNFIELD — large existing codebases with history and quirks — and an unguided agent re-derives the layout differently every run; Andrea calls the inconsistency "a box of chocolates." The mature answer feeds every agent a shared, consistent picture up front — the real architecture, the language guidelines, the rules that carry intended design — and then runs verification as THREE LOOPS: the AGENTIC loop, where the agent iterates against checks inside its own sandbox (better output, fewer tokens, less risk before anyone looks); the CI VERIFICATION loop — zero-trust, multi-layered review with a hard quality gate at sandbox exit, so fixes merge at high volume with confidence; and the CODE MAINTENANCE loop — a background patrol steadily remediating old debt, because Sonar measured that tangled A.I.-written code left to evolve for half a year starts costing more TOKENS every session, the model re-spending effort to understand the mess it made. Sonar\'s own 2026 stack maps onto the loops: a Verification Engine of thousands of deterministic rules across 40+ languages (SonarQube, and the newer Sonar Vortex) serving the agentic loop; an explainable A.I. Code Review (from the Gitar acquisition) in the CI loop, whose real value is making each finding something a person can reason about inside a 5,000-line change; and a Remediation Agent working down the backlog. Over it all sits the practical question: how much verification does THIS change need? The answer is a DIAL set by what failure would cost — a typo on a marketing page and a bug in a payment system deserve different scrutiny; a single comma can be the difference between a working operating system and a crash — so mature teams let agents carry every change as far toward verified as guardrails allow, merge the low-risk work automatically, and route the risky work to human eyes; setting those tiers deliberately is the emerging craft. Now look at this house, because you are standing inside a working example. DR-0076 wrote the doctrine: no claim without evidence; deterministic gates that fail the build; every gate PROVEN-TO-CATCH before its green means anything; measure, don\'t claim; characterize before you change; reality-trace before you build; independent adversarial verification for high stakes; provenance and honest uncertainty; the human governs the bright lines. The lane runs it live: 8,000+ tests and hard gates on every change, auto-merge only on green, the hold label as the Governor\'s hand, the OpsBoard showing the whole of it in real time — open Admin and watch this lesson operate. Even this lesson was verified: the house went past the article to its primary sources; four claims came back CONFIRMED with their real numbers, now pinned in the test suite so a drifted figure fails the build; one famous claim FELL as folklore — and that finding IS the teaching: even an article about verification carried a claim that dissolved under "Prove all things." AND THE LESSON HAPPENED TO ITS OWN AUTHORS — 2026-08-25, THE DAY THIS PAGE WAS REBUILT, WRITTEN IN AS A CASE STUDY. Twice in one morning the doctrine ran live on this platform. Case one: the delivery lane\'s auto-merge carried a parked build past the Governor\'s hold label and into production — the merge was armed before the brake landed, and nothing ever re-checked the brake; a gate everyone trusted had never been PROVEN to catch that exact race. The watchers caught it within minutes: production was reverted, the restore verified by its exact commit signature on a real deploy, and the lane was rebuilt the same hour so the hold label now DISARMS an already-armed merge — and the fix itself shipped through the very gates it repaired. A green pipeline is not a faithful pipeline until its brakes are proven to catch. Case two: the first version of this very lesson. The A.I. distilled the Governor\'s rich source material into a tidy summary, reviewed its own work, found it good, and shipped it — and its own review shared its own blind spots, exactly the failure the independence movement above warns about, running in prose instead of code. The independent witness — the human governor — read it and searched it: "I gave more information in the prompt than that whole lesson... Word first?!" The machine\'s witness of itself seemed just until the neighbour came and searched him — Proverbs 18:17, no longer an illustration but an event with a timestamp. This rebuilt page — the Word leading, every source carried, every quote and number pinned so drift fails the build — is the correction, shipped the same day with receipts. Verification is not a topic this course teaches; it is the covenant this house keeps, and twice today it held because a human witness stayed in the loop. The article ends where the Word begins: as writing gets cheap, JUDGMENT — knowing what to build, and whether to trust it — becomes the builder\'s real work, and judgment is the Word\'s home field. "Prove all things; hold fast that which is good." (1 Thessalonians 5:21, KJV). What proves good, this house holds fast; what does not, it names and drops — "let God be true, but every man a liar" (Romans 3:4).',
    facilitator: {
      talkingPoints: [
        'WORD FIRST: verification is the Maker\'s pattern — Genesis 1 is a build log with six in-progress inspections and a final whole-work review (1:31). "Prove all things" (1 Thess 5:21) is the command form; Romans 3:4 is the ground.',
        'The ten movements of the deep lesson: the Word\'s standard → the shift (DORA) → feeling vs measurement (METR + Jeremiah 17:9-10) → trust earned in degrees (Luke 14:28-30) → the filter stack, micro → false alarms and the CAP-like trade → the pipeline, shift left, and the 100x folklore (Proverbs 22:3) → the independence law (Deut 19:15; 2 Cor 13:1; John 5:31; Prov 18:17; 1 John 4:1) → the measured damage (Veracode, GitClear) → the modern stack, the risk dial, and our own lane.',
        ...
        'The same-day case study (2026-08-25, in the lesson\'s tenth movement): the lane merged past the Governor\'s hold (a brake never proven to catch that race — caught, reverted, rebuilt so hold DISARMS an armed merge), and this lesson\'s own first version passed the machine\'s self-review and fell to the human witness\'s search — Proverbs 18:17 as an event with a timestamp.',
      ...
      discussionPrompts: [
        ...
        'Read the case study in the tenth movement: the machine reviewed its own work and found it good, and the human witness searched it and it fell. Where do you grade your own work — and who is your independent witness?',
      ],
    },
  },
];
