...
export const RESEARCH_FINDINGS = Object.freeze([
  ...
  },
  {
    ...
    verdict: 'watch',
    why: 'Statelessness removes the exact objection that kept a sovereign MCP server off the table: no session affinity means an MCP endpoint deploys like any DR-0132 FastAPI route — plain request/response behind Caddy on the NAS (which can now route/authorize on the Mcp-Method/Mcp-Name headers), or even a Cloudflare Pages Function on the transport layer the app already runs — and the official PYTHON SDK speaking it day-one means the sovereign lane’s own language is first-class. Two spec patterns are the house’s own Ways said back: explicit visible state handles over transport-hidden sessions (SWIMLANES’ durable-state-not-session-memory), and MRTR elicitation — a tool confirming with the user before a destructive act — is preview-then-execute in protocol form. The named flip-to-staged trigger: the first real need to expose house tools to agent sessions as tools (the cloud sandbox has no LAN route — a Funnel-fronted stateless MCP server would be the sessions’ hands on NAS state), once mainstream clients speak it.',
    constraint: 'No change just because (DR-0143): the house runs MCP only as a CLIENT today and the sovereign lane’s needs are met by the Supabase-bus + box pattern; adopting server infrastructure ahead of a demanded workload is the DR-0140 inversion. It is also a breaking revision — old-spec servers/clients coexist through the twelve-month window, so nothing is rewritten until the client side the house actually uses (including Claude) speaks it in production.',
    reReview: '2026-08-25',
  },
  {
