...
export const ARI_STANDING_DUTIES = [
  ...
  { key: 'captions', duty: 'Keep the Word captioned — every service video carries a sovereign, timestamped caption track (video_transcripts.vtt) OWNED by us, not borrowed from YouTube: live at service time, perpetually on each new upload, and backfilled across the archive. Coverage is a MEASURED number (captioned videos / the real corpus); a gap is surfaced for the caption pipeline, never painted, and low coverage files a concern (accessibility is an obligation, not a nicety).', drRef: 'DR-0137' },
];
