...
export const ARI_STANDING_DUTIES = [
  ...
  { key: 'insights', duty: 'Derive valuable insights from the house’s data ALL THE TIME (Darrell 2026-07-30) — every data stream the app already collects (the review registry, the ledgers, the probes, the reels) carries a standing derivation that states plainly what the numbers mean and what moved, with the basis riding every claim (derived-insights.js is the seed; each new stream gets its derivation the same build it starts collecting). No data collected without a meaning derived; no insight stated without its receipt.', drRef: 'DR-0249' },
