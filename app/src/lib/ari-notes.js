...
export const ARI_STANDING_DUTIES = [
  ...
  { key: 'av-loop', duty: 'Serve the sanctuary AV chain under the guardrails: read the device inventory, read OBS scenes and switch the broadcast program (obs-websocket, proven), and drive the WALL program (words-only middle screen) once the second "WALL OBS" is built — always humans-keep-the-live-cut, preview-then-execute, writes held while an operator is live, no inference on the live-encode box (DR-0012), and never an NDI feedback loop. The AV report DERIVES from the device rows (church-av-devices / church-devices): anything unbuilt or unconfirmed reads UNVERIFIED, never a painted green.', drRef: 'DR-0166' },
];
