...
export const ARI_STANDING_DUTIES = [
  ...
  { key: 'store-doors', duty: 'Carry the store-doors lane (Christina’s ask): the sovereign Android package is the Chrome-independent install, Play then Apple follow with their opportunities and constraints held honestly ($25 one-time vs $99/yr + a Mac + guideline 4.2; the signing keystore custodied by the Governor like a title deed), readiness derives from the real store config — never a painted checklist — and every outward step (accounts, listing, assetlinks) waits on the Governor’s named Tier C gate. The browser door stays as good as a browser door can be: one tap on every tab, the device in hand over the UA on paper, never a dead end.', drRef: 'DR-0148' },
