...
export const DATASYSTEMS_MODULES = [
  ...
  // ---------------------------------------------------------------------------
  // Grounds: sanctuary-wall-novastar-vx1000-signal-path.md (9ft x 12ft, 1.9mm
  // pitch, ~1920x1440 canvas, VX1000, NOT NDI-direct, native-res authoring rule),
  // display-targets.js. SME flag: exact pixel map from NovaLCT / module datasheet.
  {
    id: 'dsi8-the-led-wall',
    title: 'The LED video wall and the NovaStar processor',
    bigIdea: 'The sanctuary\'s big picture comes from an LED video WALL — a grid of tiny LEDs, about 9 feet tall by 12 feet wide, very fine (1.9mm between pixels). It is driven by a NovaStar VX1000 processor, the box that takes a video signal and lights up the wall correctly. The single most important thing a volunteer must understand: the wall is fed by a VIDEO CABLE (HDMI/DVI) through the VX1000 — it is NOT fed directly by the network video (NDI). And images must be made at the wall\'s full resolution, because a fine wall will mercilessly show a blurry, upscaled picture.',
    ...
    levels: {
      teen: 'The big screen at the front is not one giant TV — it is an LED WALL, a grid made of thousands of tiny lights (LEDs). Ours is about 9 feet tall and 12 feet wide, and the lights are packed really close together (1.9 millimeters apart), which makes the picture sharp. To show anything on it, a video signal has to go through a special box called a NovaStar VX1000 — that box takes the picture and spreads it correctly across the whole grid of lights. Here is the thing volunteers MUST get right: the wall is fed by a VIDEO CABLE (like HDMI), through the VX1000. It is NOT fed by the network video that floats around on our church network (that is called NDI, and it is great for cameras and side screens, but it does not plug straight into the wall — it would have to be converted to a cable first). And because the lights are packed so tightly, a low-quality, stretched-out image looks blurry and bad. So always use big, high-quality images made for the wall\'s real size.',
      senior: 'The sanctuary\'s primary output is a fine-pitch LED video wall: roughly 9 ft high by 12 ft wide at a 1.9 mm pixel pitch, with a documented design canvas around 1920 x 1440 (about 4:3). It is driven by a NovaStar VX1000 — an all-in-one video processor and controller that takes an input signal and maps it across the LED cabinets. The signal path that volunteers must internalize: presentation PC -> HDMI/DVI cable -> VX1000 -> the LED grid. Use the higher-bandwidth DVI / "HDMI 4.1" inputs for a ~1920x1440@60Hz signal (plain HDMI 1.4 tops out around 1080p60). The crucial correction: the VX1000 has NO native NDI input. NDI (the network video transport that carries camera and production feeds around the production LAN, and feeds the switcher and side screens) does NOT feed the wall directly — to put an NDI source on the wall you must first decode it to HDMI into a VX1000 input. So there are two lanes: the WALL lane (HDMI/DVI through the VX1000) and the NDI production-LAN lane (IP routing for cameras, switcher, streams, side screens). Image-quality rule (binding): author and render at the wall\'s native resolution and aspect, full-bleed; feed source images at or above native resolution and never upscale a small image onto a 1.9 mm wall, which would show every soft edge. SME flag: the exact pixel map comes from the NovaStar screen configuration (NovaLCT) and the LED module datasheet — the ~1920x1440 figure is the documented design estimate; the true count snaps to the physical cabinet grid, so confirm it before treating it as exact.',
    },
    quiz: {
      questions: [
        {
        ...
        },
        {
          ...
          options: [
            ...
            'Because the fine 1.9 mm pitch shows every soft edge — a small, upscaled image looks blurry on the wall',
    ...
    lesson: 'The big picture at the front of the sanctuary is one of the most visible things the tech team controls, so every volunteer should understand it plainly. It is not a single giant television — it is an LED video WALL, a grid built from many small modules packed with tiny LED lights. Ours is roughly 9 feet tall and 12 feet wide, with a very fine pixel pitch of 1.9 millimeters (the lights are packed close together, which is what makes the picture look sharp), and a documented design canvas of about 1920 by 1440 pixels. To show anything on that grid, a video signal has to pass through a dedicated box: a NovaStar VX1000, which is an all-in-one video processor and controller that takes an incoming picture and maps it correctly across all the LED cabinets. Now the single most important thing to get right, because it is the most common misunderstanding: the wall is fed by a VIDEO CABLE — HDMI or DVI — running into the VX1000, and from the VX1000 to the wall. It is NOT fed directly by NDI. NDI is the network video that travels around our production network and carries the camera feeds and the side-screen content and feeds the switcher; it is excellent for that, but the VX1000 has no NDI input, so to get an NDI source onto the wall you would first have to convert it to an HDMI cable. Picture it as two lanes: the WALL lane (a cable through the VX1000) and the NDI production lane (network routing for cameras, switcher, streams, and side screens). For the cable, use the higher-bandwidth DVI or "HDMI 4.1" inputs to carry the full ~1920x1440 picture at 60 frames per second, because plain HDMI 1.4 tops out around 1080p. The other rule that protects the look of everything you put up is about image quality: always author and display images at the wall\'s native resolution and aspect, edge to edge, and feed source images that are at least as large as the wall\'s real resolution. Never take a small image and stretch it big, because a 1.9 mm wall will reveal every soft, blurry edge without mercy. Finally, an honest SME flag: the EXACT pixel map of the wall comes from the NovaStar screen-configuration software and the LED module datasheet — the roughly 1920x1440 figure is the documented design estimate, and the true count snaps to the physical grid of cabinets, so confirm it from the real configuration before treating it as exact. Scripture gives the wall its purpose: write the vision and make it PLAIN, so the one who reads it may run (Habakkuk 2:2), and declare His glory (Psalm 96:3). Clarity on that wall is not vanity — making the vision plain and declaring His glory clearly is the whole reason it is there.',
    facilitator: {
      talkingPoints: [
        'The front screen is an LED WALL (~9 ft x 12 ft, 1.9 mm pitch, ~1920x1440 canvas), driven by a NovaStar VX1000 processor/controller.',
        ...
        'Use DVI / "HDMI 4.1" for ~1920x1440@60Hz (plain HDMI 1.4 tops ~1080p60). To put NDI on the wall, decode it to HDMI first.',
        'Image rule (binding): author + render at native resolution, full-bleed; feed high-res source; never upscale a small image onto a 1.9 mm wall.',
        'SME flag: exact pixel map comes from NovaStar config (NovaLCT) + module datasheet; ~1920x1440 is the design estimate — confirm before treating as exact.',
      ...
      discussionPrompts: [
        ...
        'Why does a fine 1.9 mm wall punish a low-resolution image so badly?',
  ...
  // ---------------------------------------------------------------------------
  // Grounds: relationships.js (3 relationship types + can/can't matrix), child-
  // safety structural (maxGrant, finance/security locked-deny), tenant-portal,
  // guardian-child, steward/governor gates (surfaces.js, family-gated).
  {
      ...
      questions: [
        {
          ...
          answer: 1,
          explain: 'Access is a function of relationship (guardian-child, family, landlord-tenant) and role (steward, governor, etc.), enforced by the permission model and RLS.',
        },
        {
