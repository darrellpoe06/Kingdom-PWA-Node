...
export async function assignLesson({ clientEmail, lesson, track = 'client', dueOn = null, note = '' }) {
  ...
  try {
    ...
    if (error) return fail('write-error', error.message);
    return { ok: true, row: data };
  } catch (e) { return fail('network-error', (e && e.message) || 'the connection failed'); }
}
...
export async function listMyAssignments() {
  try {
    ...
    if (error) return { ok: false, reason: 'read-error', message: error.message, rows: [] };
    return { ok: true, rows: data || [] };
  } catch (e) { return { ok: false, reason: 'network-error', message: (e && e.message) || 'the connection failed', rows: [] }; }
}
