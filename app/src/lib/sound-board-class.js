...
// the worship team and the CHOIR, the three services of the operator (BEFORE /
// DURING / AFTER a service), and OUR ACTUAL CONSOLE — a Yamaha QL-series DIGITAL
// board (scenes/snapshots, Selected Channel EQ/dynamics, the QL Editor + StageMix
// remote, and Dante/USB multitrack for virtual soundcheck). The craft is taught
// around the digital, software-controllable workflow the team really uses — not
// analog knob-only. It rides the SAME shared Learn engine as the other
...
export const SOUND_BOARD_META = {
  ...
  footer: '_Built for the Church of the Living God sound team · taught with the church’s own sound engineer · on PoeTech. Skill is a calling (1 Chronicles 15:22) and an offering (Colossians 3:23) — we mix so the Word is heard and the worship carries, decently and in order. Sovereign and local. The A.I. mix helper starts by suggesting (you apply every change), and grows over time — in earned, bounded stages — toward doing more of the mixing, ALWAYS with a person on the loop who can take over instantly. You are never removed from the board._',
};
...
export const SOUND_BOARD_MODULES = [
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    bigIdea: 'Running sound is three jobs, not one. BEFORE: you prepare — line-check every input, gain-stage, build the monitor and house mixes, ring out feedback, and save a scene, so you start the service ready instead of scrambling. DURING: you serve in real time — ride the faders, mute open mics, follow the flow from song to sermon, watch levels for the room and for hearing safety, and stay calm. AFTER: you grow — listen back to the recording (a virtual soundcheck from the multitrack), note what was muddy or harsh or buried, and bring it to the next service better. These same three windows are where the church’s A.I. mix helper works: today it ASSISTS — it analyzes and suggests, and the human operator decides and applies every change; over time, in earned and tightly-bounded stages, it can do more of the moving — but always with you on the loop, able to take over instantly, and never past the hard safety limits. You are never removed from the board.',
    ...
    facilitator: {
      ...
      discussionPrompts: [
        ...
        'How is the after-review loop like the way the whole platform is meant to get better forever — and how does keeping a human on the loop honor both the worship and the craft?',
      ],
    },
  },
  // ---------------------------------------------------------------------------
  {
    id: 'snd8-our-digital-console-yamaha-ql',
    title: 'Our console: the Yamaha QL — scenes, Selected Channel, the editor, and virtual soundcheck',
    bigIdea: 'Our board is a Yamaha QL-series DIGITAL console, and that changes how you work. Everything you have learned — gain, EQ, feedback, monitors, the mix — is now RECALLABLE: you edit one channel at a time in the Selected Channel section and the touchscreen, you save the whole desk as a SCENE you can recall in a second, you can mix from an iPad anywhere in the room (QL StageMix) or a laptop (QL Editor), and you can record the service as multitrack (Dante/USB) to do a VIRTUAL SOUNDCHECK — re-mix the recorded service with the band gone. The digital board turns mixing from “set it and remember it” into “prepare it, save it, recall it, refine it.”',
    inApp: 'Make a scene and recall it. On the QL, set up your inputs the way you like, then SAVE it as a named scene. Change a few faders and EQs, then RECALL your saved scene and watch the whole desk snap back. Then open QL StageMix on the iPad (or QL Editor on the laptop), confirm you can see and move the same channels remotely, and find where multitrack recording lives for a future virtual soundcheck.',
    anchor: {
      ref: '1 Corinthians 14:40; Proverbs 21:5',
      theme: 'All things decently and in order — a saved scene is literally order made recallable, so each service starts prepared, not scrambled. And the plans of the diligent lead to abundance: the digital board rewards preparation, recall, and refinement over winging it.',
    },
    benefits: [
      'You start every service in seconds by recalling a known-good scene — no rebuilding from scratch.',
      'You can walk the room and mix from an iPad, hearing the house where the congregation actually sits.',
      'A safety net: if a mix goes sideways, one scene recall restores a known-good state instantly.',
      'Virtual soundcheck — refine the mix from a recording with no one on stage, so Sunday is dialed before anyone arrives.',
      'You speak the same digital language the A.I. mix helper uses (scenes, Selected Channel EQ/dynamics), so its suggestions map straight onto your board.',
    ],
    levels: {
      teen: 'Our board is a Yamaha QL, and it’s DIGITAL — which is great news, because the computer remembers everything for you. Instead of a hundred physical knobs, you pick a channel (the SEL button) and edit just that one in the “Selected Channel” area and on the big touchscreen — its EQ, its dynamics, all of it. When you’ve got the whole board how you want it, you SAVE it as a “scene,” like saving a game. Next service, you RECALL that scene and the entire board jumps back to your setup in one second. You can even mix from an iPad walking around the room (an app called QL StageMix) so you hear what the people hear. And you can record the whole service and play it back later to practice mixing it with nobody on stage — that’s a “virtual soundcheck.” So the digital board isn’t scary; it’s the easiest kind to learn, because it saves and recalls your work, and you can always recall a good scene if things get messy.',
      senior: 'The QL/CL workflow is built on Selected-Channel editing + scene recall, and mastering it is most of operating the desk well. Selected Channel + the central touchscreen + TOUCH AND TURN gives you deep per-channel control (parametric EQ — the close-up was on that view — dynamics, sends) one channel at a time, with channel libraries to store and recall per-input/per-voice starting points. Scenes/snapshots capture the whole console state for instant recall — build a service’s scene set (or a solid default) so you start prepared and can restore a known-good state mid-service if needed (a real safety net). Remote control is native and is how the team already works: QL Editor (Win/Mac) for setup on or offline, QL StageMix (iPad) for mixing from the floor/monitor world, ProVisionaire for broader control — all over the network, with separate IPs for console remote, remote head-amp, and the Dante audio network. Dante (standard on QL) plus USB recording gives you multitrack capture for virtual soundcheck: record the service, then mix the playback with the stage empty to refine EQ/levels/scenes before the next one — and that same multitrack is what the A.I. mix helper analyzes for its AFTER review and what it rehearses on before any change touches a live service. This is also why the A.I. integrates cleanly: it reads console state (EQ curves, levels, meters, scenes) over that existing remote channel and expresses its per-voice EQ suggestions in the QL’s own channel-EQ terms — and in later, earned, tightly-bounded stages it can apply moves through that same channel, always within scene bounds, always with your one-touch takeover. (Confirm whether ours is a QL1 or QL5 by the channel count — QL1 is 32 mono inputs, QL5 is 64; it changes layout and capacity, not the workflow.)',
    },
    quiz: {
      questions: [
        {
          q: 'What does saving and recalling a SCENE on the digital console do for you?',
          options: [
            'It records the audio of the service',
            'It stores and instantly restores the whole console state — your gains, EQ, levels, routing — so you start prepared and can recover a known-good mix',
            'It turns the board off',
          ],
          answer: 1,
          explain: 'A scene/snapshot captures the entire desk state for one-second recall — preparation and a safety net, both. (It is not the audio recording; that is multitrack capture.)',
        },
        {
          q: 'What is a “virtual soundcheck,” and what makes it possible on the QL?',
          options: [
            'Guessing the mix without testing',
            'Recording the service as multitrack (via Dante/USB) and re-mixing the playback with the stage empty, to refine the mix before the next service',
            'A StageMix feature for muting channels',
          ],
          answer: 1,
          explain: 'Multitrack capture (Dante/USB) lets you mix the recorded service with no one on stage — and it is also what the A.I. helper reviews and rehearses on before any live change.',
        },
        {
          q: 'How does the team adjust this console besides the physical board?',
          options: [
            'Only by the physical knobs',
            'Over the network — QL Editor on a computer and QL StageMix on an iPad — which is also how the A.I. helper reads state and (in earned, bounded stages) applies moves',
            'It cannot be controlled remotely',
          ],
          answer: 1,
          explain: 'QL Editor + StageMix control the desk over the network today; that same remote channel is how the A.I. reads console state and, in later bounded stages, applies changes — always with human takeover.',
        },
      ],
    },
    lesson: 'Everything in the first seven lessons — the signal chain, gain, EQ, feedback, monitors, the mix, the before/during/after rhythm — is true on any board. This lesson is about OUR board, because it shapes how you actually work: it’s a Yamaha QL-series digital console, and digital changes the game in your favor. On an analog desk, every setting is a physical knob you must set by hand every time and remember between services. On the QL, the computer remembers. You edit one channel at a time: press its SEL button and shape it in the Selected Channel section and on the central touchscreen — its parametric EQ (the close-up of our board was on exactly that screen), its dynamics, its sends — with the TOUCH AND TURN encoder for fine control. You can store a channel’s setup in a channel library and recall it for a similar voice or input later. And the biggest gift of digital: SCENES. When the whole board is how you want it, you save it as a named scene, and next service you recall it and the entire desk — every gain, EQ, level, and route — snaps back in about a second. Build a good default scene (and scenes for special services) and you start every Sunday prepared instead of from scratch; and if a mix ever goes sideways mid-service, recalling a known-good scene is your instant safety net. The QL is also controllable over the network, which is how the team already works: QL Editor on a Windows/Mac laptop for setting up on or offline, QL StageMix on an iPad for mixing while you walk the room and hear what the congregation hears, and ProVisionaire for broader control — over the network, with separate addresses for the console remote, the remote head-amp, and the Dante audio network. That Dante network (standard on the QL), plus USB recording, gives you multitrack capture, which unlocks the virtual soundcheck: record the whole service, then play the multitrack back into the console and mix it with the stage completely empty — so you can refine EQ, levels, and your saved scene for next time without asking the band to stand around. Two things tie this lesson back to the rest of the track. First, this is where the AFTER-review loop from Lesson 7 actually lives — your recording is the material you grow from. Second, this is why the church’s A.I. mix helper fits our setup so cleanly: because the board is already software-controllable, the helper reads the console’s state (EQ curves, levels, meters, scenes) over that same remote channel and gives its per-voice EQ suggestions in the QL’s own channel terms; today it suggests and you apply, and in later, earned, tightly-bounded stages it can make moves through that channel — always inside scene bounds, always with your one-touch takeover, never past the hard limits. (One thing to confirm: whether ours is a QL1 or a QL5 — the channel count tells them apart, QL1 with 32 mono inputs and QL5 with 64 — which changes capacity and layout, not the workflow you just learned.) The order Scripture calls for — “all things decently and in order” (1 Corinthians 14:40) — is, on a digital console, literally something you save and recall; and “the plans of the diligent lead surely to abundance” (Proverbs 21:5) is the whole spirit of preparing, saving, recalling, and refining your work instead of winging it each week.',
    facilitator: {
      talkingPoints: [
        'Our board is a Yamaha QL-series DIGITAL console — digital means the desk REMEMBERS: Selected-Channel editing + the touchscreen + channel libraries, not analog knob-by-knob.',
        'Scenes/snapshots = the whole console state saved + recalled in ~1 second: start prepared, and recall a known-good scene as an instant mid-service safety net.',
        'Remote control is how they already work: QL Editor (laptop) + QL StageMix (iPad, mix from the floor) + ProVisionaire, over the network (separate IPs for console remote / head-amp / Dante).',
        'Dante (standard) + USB multitrack → virtual soundcheck: re-mix the recorded service with the stage empty; it is also the A.I. helper’s AFTER-review + rehearsal material.',
        'The A.I. integrates because the board is software-controllable: it READS state + suggests per-voice EQ in the QL’s own terms today; earned bounded stages can apply moves — always scene-bounded, always one-touch human takeover.',
        'Confirm QL1 (32 mono in) vs QL5 (64) by channel count — changes capacity/layout, not the workflow. Order you can save + recall (1 Cor 14:40); diligence rewarded (Prov 21:5).',
      ],
      howToRun: 'Pray + read the anchor (3): read 1 Corinthians 14:40 — order, now savable + recallable. | The big idea (10): show Selected-Channel editing + the touchscreen; explain digital "the desk remembers." | Go deeper (15): save a scene, change the board, recall it; open QL Editor/StageMix; locate multitrack record + explain virtual soundcheck. | Hands-on at the board (20): learner builds + saves a default scene, recalls it, and mixes a channel from StageMix on the iPad. | Check yourself + next service (7): plan a virtual soundcheck from a recorded service, and confirm QL1 vs QL5 by the channel count.',
      discussionPrompts: [
        'How does saving and recalling scenes change the way you prepare for a service?',
        'When would mixing from the iPad out in the room beat mixing from the booth?',
        'What could a virtual soundcheck let you fix that you could never rehearse with the band present?',
      ],
    },
  },
];
...
export const SOUND_BOARD_TUTOR_META = {
  ...
  posture: 'Teach REAL, accurate live-sound craft — the signal chain, gain staging, the EQ frequency ranges, taming feedback, monitors vs the house, mixing the worship team and the CHOIR, the before/during/after rhythm, and our actual Yamaha QL-series DIGITAL console workflow (scenes/snapshots, Selected Channel EQ/dynamics, the QL Editor + StageMix network remote, Dante/USB multitrack for virtual soundcheck) — matched to the learner\'s experience (a nervous first-timer vs a seasoned operator) and pitched plainly. Be relentlessly practical and encouraging; the board is intimidating, so reduce fear and build confidence step by step. Ground the heart of it in service: the operator mixes so the WORD is heard and the worship carries, skillfully (1 Chronicles 15:22; Psalm 33:3) and decently and in order (1 Corinthians 14:40), offered to the Lord (Colossians 3:23). Cite Scripture by reference (do not quote a translation you are unsure of; never invent or paraphrase a verse as if quoting it). Be HONEST and verifiable: rules-of-thumb (like a ~ -18 dBFS gain target) are guides, not laws — say so. SAFETY: today the A.I. mix helper is ASSISTIVE — it analyzes and SUGGESTS, and the human operator decides and applies every change. Darrell\'s longer aim is staged, EARNED autonomy (supervised, then autonomous live mixing) that is ALWAYS human-on-the-loop — fenced by hard limits (a level/SPL ceiling, automatic feedback hard-stop, smooth rate-limited moves, staying within a known-good scene, every move logged) and an instant one-touch human takeover, never unbounded and never unwatchable. The human\'s skill stays permanently essential (they supervise, co-mix, and can take over instantly), so encourage the learner that mastering this never stops mattering; never tell a learner the A.I. will simply replace them or that it changes a live mix without those bounds and a human on the loop. You run on the church\'s own sovereign, local A.I. (its RTX 4070 machines); you can be wrong — tell them to verify what matters with their ears and their sound engineer.',
};
