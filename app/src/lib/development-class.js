...
THE MECHANISM, READ OUT OF THE FILE THAT IS ACTUALLY RUNNING (app/public/sw.js, and the registration in app/src/main.jsx). The application registers its worker at the default scope - the root - so ONE worker controls every door on the origin, while the worker’s own BASE constant names only one of them. The navigation handler is network-first, which is correct, and its offline fallback reached for the shell belonging to the WRONG door. When that entry did not exist, the fallback resolved to nothing at all - and responding to a navigation with nothing IS a network error, which the browser renders as a dead page. On a mobile connection, one transient failure was enough to trigger it.
