...
### REV-0011 · Efficiency pass — per-course monolith wiring is the next dedupe target
- **Date:** 2026-07-06
- **Surface:** How the work ran this session — adding self-paced Learn courses (wealth, pride, table)
- **Type:** orchestration
- **Status:** open
- **Findings:** Continuous-efficiency pass (DR-0109) on the course-adding workflow. REALIZED WIN this round: the Table series added ZERO new verse-script code — it reused verses already in the store, and where new verses were needed the unified scripts/append-verses.mjs (REV-0010) meant one registry entry, not a copied ~100-line script. The fix is paying off. NEXT TARGET (tracked, not yet shipped): adding a self-paced course still copies ~40 lines of near-identical MONOLITH wiring each time — an import block, a submitInterest closure, a roster line, a ~28-line course-descriptor literal, a helperTagFor branch, and an extraCourses array entry. Seven self-paced courses now share that exact shape (livingLessons, wealth, pride, table, soundBoard, worldIssues, datasystems). A `selfPacedCourseDescriptor()` factory in a module (church-classes.js or a new lib) that takes the per-course pieces + the shared closures would collapse each to ~10 lines AND move the boilerplate OUT of the monolith into a module — serving both "the next course is cheaper" and the module-growth direction (DR-0103). WHY TRACKED, NOT SHIPPED NOW (DR-0109 clause): it touches all seven live course wirings + the ChurchLearn contract at once — real regression surface best refactored with its own focused verification (a course-by-course diff + a full render pass), not at the tail of a long multi-teaching session. **re-review: 2026-07-13.**
