...
> - **RAM ceiling: 62 GB ECC ACTUAL (corrected 2026-05-26).** Earlier sessions documented "32 GB max" per Synology's official compatibility list. Live `free -h` on the DS1621xs reports 62 GB physical + 39 GB swap. Unit is populated past Synology spec and runs stable. Scoring below uses the real 62 GB pool. This is a meaningful architectural unlock — see the "What changed" callout below.
