...
- **Property scope keys on `rentals.slug`, NOT the uuid PK** (lane-confirmed 2026-07-01 from `rentals-sync.js:70 slug: item.id`). `rentals.slug` is the app's local door id (text); `property_notes.rental_ref` and `rental_tenancies.rental_ref` are both that slug. The framework keys `pm_property_view` / `pm_renter_view` / `learner_property_view` / assignment `scope_ref` on **`slug`** → `scope_ref == rental_ref` with zero joins (the lane's recommended option a). `pm_assigned_to_rental(uuid)` stays as a wrapper that resolves uuid→slug. **The owner assigns using `rentals.slug`.** Caveat: a user-added door not yet synced has no slug/uuid row → not assignable to a cloud-only external PM until it syncs (surface this in the assign UI).
- **Live tables key on `tenancy_id`.** `tenant_messages` / `tenant_maintenance_requests` (0055) reference `rental_tenancies(id)`; the framework adds `subject_assigned_to_tenancy(tenancy_id)` = `subject_assigned_to('property', rental_ref)` and PM policies on those two live tables (§4c). The external PM works the **actual** tables, scoped to assigned units.
