...
| Poe Properties pictures — walk-through camera + spoken captions | Take a photo (camera, one shot) or choose many; a caption per picture, Speak on each where the browser can hear; thumbnails in every list, full image by id on open; 1099 worker with `docs.add` files to the door he is sent to, tenant or no tenant (DR-0339) | `modules/properties/DoorTabs.jsx` (GalleryTab), `modules/properties/cloud.js` (`PHOTO_LIST_COLUMNS`, `loadPhotoImages`), `modules/properties/photo-order.js` (`pickCovers`), `lib/voice-dictation.js` · `infra/supabase/migrations-auto/0185-*.sql` | CONFIRMED (`property-photos-list-carries-no-bytes.test.js`, `properties-qr-gallery-files.test.jsx`, `properties-door.test.js`) | yes — `property_photos` (+ `thumb_path`), RLS additive delegate arms |
| Real Estate strip reads the door's cloud pictures | The same `property_photos` rows, by the record's `remoteUuid`, in the oldest→latest strip beside room / maintenance / NAS pictures (DR-0339) | `components/Rentals.jsx` (PropertyGallery) | CONFIRMED (`property-photos-list-carries-no-bytes.test.js` "the surfaces") | yes — read-only |
| The Word opens in place — VerseChips + WordInline | Any Scripture reference is a button; the verbatim KJV (hosted in-app) opens beneath the chip row or the paragraph, no navigation; prose keeps every character (DR-0340, UX-PATTERNS 2h) | `components/VerseChips.jsx`, `components/WordInline.jsx`, `lib/verse-refs.js`, `lib/video-harvest.js` (`findScriptureRefs`) · wired: TorahPatternMap, Study, ChurchLearn, ScriptureLibrary | CONFIRMED (`verse-chips.test.jsx`, `word-inline.test.jsx`) | no — reads the static KJV assets |
| Show the Word — one switch, in order, reader told | Every verse on a page opens/closes with one tap; each chip still toggles on its own; opened verses read in timeline-then-flow order; a plain hint beside the switch; the matcher tells Job from a job (DR-0341) | `lib/show-the-word.js`, `components/ShowTheWordToggle.jsx`, `lib/scripture-order.js`, `lib/video-harvest.js` (`isNotAVerse`) · on TorahPatternMap, Study, ChurchLearn, ScriptureLibrary | CONFIRMED (`show-the-word.test.jsx`, `scripture-order.test.jsx`, `word-inline.test.jsx`) | no — device preference in localStorage |
| Full profiles — one row per person | Name, picture (thumbnail only), house, ministries, a verse that opens in place, testimony, visibility (members / leaders / private); seen by whoever may already message you (`users_can_dm`); the name in a 1:1 thread opens the card; My profile on Church → Engagement (DR-0342) | `lib/profiles-sync.js`, `components/ProfileCard.jsx`, `components/MyProfile.jsx`, migration 0186 | CONFIRMED (`profiles-sync.test.js`, `profile-surfaces-render.test.jsx`, `engagement-direct-render.test.jsx`) |
| Notifications everywhere — a DM reaches the shade, the icon, the status bar | The sender's DB credentials installed from the NAS by `push-sender-credentials.yml` (dispatch; three proofs, the last on the live site); a message's audience read by PERSON (`audienceQuery`); the worker and the page set/clear the app-icon badge; `badge-96.png` status-bar glyph (DR-0343) | `.github/workflows/push-sender-credentials.yml`, `lib/push-send-policy.js`, `functions/api/push-send.js`, `public/sw.js`, `lib/dm-notify.js`, `public/badge-96.png` | CONFIRMED up to the push service (`push-send-policy.test.js`, `sw-push-handler.test.js`, `dm-notify.test.js`; the workflow's live witness) — **the phone is the last witness**, re-review 2026-09-13 |
| Seen / Delivered receipts in a 1:1 thread | `✓✓ Seen <time>` under the last of my messages once the reader opened it (a real `read_at`), `✓ Delivered` while it waits; never under theirs (DR-0343) | `lib/direct-messages.js` (`receiptLabels`), `components/DirectMessages.jsx` | CONFIRMED (`direct-messages.test.js`) |
| Every control on a member surface acts | Standing gate: no `<button>` without a handler, no `href="#"`, across every `Church*.jsx` + Pulpit, Engagement, DirectMessages, ReactionBar, MyProfile, ProfileCard, PushNotifications, LiveWorshipBar, ServiceProgram, Choir, BusMinistry; the real-Chromium journey walk per review (DR-0343) | `__tests__/member-surfaces-every-control-acts.test.js` | CONFIRMED (proven-to-catch) |
| The Love Corner door governs itself — Church → Members | The join process as the server runs it (0014 / 0081) with a live count beside each step; people grouped top-down by standing with faces; each person's MAY / MAY NOT from `ROLE_LABELS` + the `CAPABILITIES` catalog; the standing as a control; extra rights as checkboxes; Remove (two taps, `canRemove` mirrors 0130); invite by email + standing; open invites; claims to confirm. Gated by the real standing (`list_my_admin_instances`), a signed-in member sees the way in only (DR-0348) | `lib/church-members.js`, `components/ChurchMembers.jsx`, `surfaces.js` (`church-members`), one shell render line | CONFIRMED (`church-members.test.js`, `church-members-render.test.jsx`, `surface-mount-integrity`, `member-surfaces-every-control-acts`) | Live witness: the phone, re-review 2026-09-13; leader allowlist editable → 2026-09-24 |
| The person is seen: faces everywhere, the header carries the person | `useProfiles` + `preferredName` (one cached read per person); picture + chosen name on 1:1 rows, thread header, start chips, incoming bubbles, family-thread rows, the My-profile fold; `HeaderAuthButton` shows picture (or initials + `+ photo`) + name with Log out beneath and opens My profile (the upload spot) on every app; Engagement opens on Message a member, then Family thread, then Trivia (DR-0342 amendment 2026-09-09) | `lib/use-profiles.js`, `components/DirectMessages.jsx`, `components/Engagement.jsx`, `components/HeaderAuthButton.jsx` | CONFIRMED (`engagement-direct-render.test.jsx`, `engagement-voice-sections-render.test.jsx`, `header-auth-button.test.jsx`) |
| A server's failure is said in words | The sovereign `/sb` proxy turns an HTML 5xx from the edge (Cloudflare 1016 etc.) into one JSON 502 with a readable `message`; `humanizeServerError()` never prints an HTML body and names what did not happen (DR-0342 amendment 2) | `functions/_lib/funnel-proxy.js`, `lib/server-error-text.js`, `lib/profiles-sync.js` | CONFIRMED (`server-error-text.test.js`, proven on a real 530 page) |
| One person, two doors, every phone; the sender is told what the push did | `expandAudience()` widens a named recipient through `person_links` (0141) before the sender reads phones; `sendDirectMessage()` returns the push promise and the thread shows `pushReportText()` (told / no phone yet / unreachable / not set up) (DR-0343 amendment) | `lib/push-send-policy.js`, `functions/api/push-send.js`, `lib/push-announce.js`, `lib/direct-messages-sync.js`, `components/DirectMessages.jsx` | CONFIRMED (`push-send-policy.test.js`, `push-announce.test.js`, `engagement-direct-render.test.jsx`) |
