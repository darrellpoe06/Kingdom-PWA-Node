...
| The Love Corner door governs itself — Church → Members | The join process as the server runs it (0014 / 0081) with a live count beside each step; people grouped top-down by standing with faces; each person's MAY / MAY NOT from `ROLE_LABELS` + the `CAPABILITIES` catalog; the standing as a control; extra rights as checkboxes; invite by email + standing; open invites; claims to confirm. Gated by the real standing (`list_my_admin_instances`), a signed-in member sees the way in only (DR-0346) | `lib/church-members.js`, `components/ChurchMembers.jsx`, `surfaces.js` (`church-members`), one shell render line | CONFIRMED (`church-members.test.js`, `church-members-render.test.jsx`, `surface-mount-integrity`, `member-surfaces-every-control-acts`) | Live witness: the phone, re-review 2026-09-13; leader allowlist editable → 2026-09-24; Remove on the tab → 2026-09-17 |
