...
| **youth** | 11-14 | Isaiah 28:10 *line upon line* | Plain language, real examples, hands-on. Abstraction is beginning but is not load-bearing yet. Its own authored level from DR-0418 (grades 6–8); reads the teen text only while a lesson's youth level is unwritten. On the reading ladder since DR-0544 — it was authored, displayed and unmeasured before that. | 90 words · 10-min segments · break every 20 min · check after 2 |
...
Debt today: 30 inverted, 17 child levels over the 7.0 ceiling. **Where the ceiling came from, said plainly:** 7.0 is the corpus median of 2026-09-06, not an age. Flesch-Kincaid grade is a US school-grade estimate, and ages 6–10 are grades 1–5; measured against the age the band names, 82 of 153 child levels read above grade 5. No research citation maps FK grade to this audience and none is claimed (§5). L141–L154 measure 0.0–5.0, most under 2.1 — sentences so short the proxy reads near zero, which is the reason to keep treating it as a gross-case detector and not a comprehension measure (DR-0332). The comprehension control that does not depend on the proxy is the learner's own pick, now reachable inside every lesson (DR-0417 D2). **Decided 2026-09-15 (Darrell: "Yes. Nice!"):** 7.0 stays the shrink-only ratchet on the 153 lessons of that date (`knownLessons` in the baseline) and every lesson written after it is held to **5.0** — the top of the band's own grade range (`NEW_LESSON_CHILD_CEILING`, DR-0417 amendment). The 17 over 7.0 are re-authored first (`re-review: 2026-09-22`), the 65 between 5.0 and 7.0 as each lesson is touched (`re-review: 2026-10-13`). The youth band read the teen text at youth pacing when this was written; DR-0418 gave it its own authored level on 2026-09-15, which closed that question early.

**The youth band was not on the ladder at all until 2026-09-19 (DR-0544).** `BAND_ORDER` shipped 2026-09-06 as child → teen → senior, the youth band arrived with L81, and it was never added — so `measureLesson` produced no youth figure and `isInverted` had none to compare. For two weeks the app displayed a youth level that nothing in this section measured, while the band-fill pass was authoring eighty more of them. Adding the rung made **30 of the 115 lessons carrying a youth band** visible as inverted, every one the same shape: the **younger** band reading **harder** than the teen band above it. Those 30 are recorded as shrink-only debt and a new youth inversion now fails the build. The `inverted` list grew once, on that date, because the gate widened — not because the corpus worsened, and the baseline's own `note` says so. L129 and L130, the two lessons this section pins as the clean standard, were **re-authored rather than exempted** (L129 youth 5.8 → 5.2 against teen 5.6; L130 6.0 → 5.1 against 5.5). `re-review: 2026-10-10` — report how many of the 30 remain, from the committed baseline.
