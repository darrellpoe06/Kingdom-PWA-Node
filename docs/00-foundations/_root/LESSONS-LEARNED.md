...
- **P24 — A defensive merge inherits its BASE; merge partial saved state over the owner's baseline, never over whatever happens to be mounted.** The `{...current, ...saved}` pattern quietly makes "whatever is on screen right now" the source of truth for every key the snapshot lacks. On a public host the mounted world is the anonymous-safety DEMO — so a signed-in owner's sparse snapshot inherited demo money rows, which then persisted as owner data and re-hydrated forever. Pass the merge an explicit baseline that matches the OWNER (their seed / empty world), and scrub provenance-marked sample rows from any real world on load. Sibling of P14 (the trust boundary changes at sign-in) applied to the merge base instead of the gate. (Extracted: 2026-07-05 Money-tab reconciliation incident.)
...
### 2026-07-05 — Big Picture Money showed the Reeves demo's "+$1k" over the owner's own $0 entities; a defensive merge inherited its base from whatever was mounted

**Trigger:** Darrell's screenshots from the fold: Big Picture → Money read NET CASH FLOW +$1k while all four of his entity cards read $0 inflow, no tab agreeing with another — and tapping Books → Tx left a bare blank screen ("Tx made the screen white").
**Detection path:** Darrell, from the running app on poetech.us (signed in, BUILD A7CB994). The header's "SAMPLE · FAMILY OF 4" badge next to HIS entities (old names "Poe Properties / PoeTech / TLC Therapy Solutions") was the tell that two worlds were mixed.
**Root cause(s):** **(a)** On a public host the app mounts the Reeves demo for anonymous safety; the signed-in hydration then ran `setData(d => ({...d, ...parsed.data}))` — a defensive merge whose base was the MOUNTED DEMO, so every key the owner's snapshot lacked silently kept demo values. The hand-kept money spine (`inflows`/`outflows`/`meta`) is not table-synced, so the demo's +$1,100/mo (salaries tagged to the demo-only `e-family`) rode in, counted in the all-entities total, and appeared on no entity card. The save effect then persisted the merged residue as owner data (and the snapshot-push guard checks SEED provenance, not DEMO — so it could publish to the family cloud snapshot). **(b)** `stripSeedScaffolding` filtered `inflows.rentals` but never `inflows.salaries` — half the class was already handled, the sibling list missed. **(c)** The Tx blank screen matches the known deploy-skew signature (2026-06-30): a stale shell requests a replaced lazy-chunk hash, and the chunk-heal's silent `location.reload()` leaves an unexplained blank page while the fresh shell fetches.
**What worked:** Reconstructing the exact screen state from the screenshots (fmtCompact(+1100) = "+$1k"; the demo releaseLabel; the $0 cards) before touching code — the arithmetic identified the polluting dataset precisely.
**What didn't work:** Surfaces that silently disagreed: nothing anywhere said "this total includes rows no card shows."
**Principle(s) extracted:** P24 (below).
**Forward architectural fix:** (1) The merge is extracted to `lib/snapshot-hydration.js` and the signed-in path passes the OWNER'S baseline (family SEED / EMPTY_WORLD), never the mounted demo; (2) `scrubDemoResidue` drops demo-provenance rows from every list including both inflow lists, and a demo `meta.releaseLabel` resets the non-provenanced remainder (meta, outflows) to the baseline — healing already-polluted snapshots on next load; (3) a new ledger-integrity check (`entity-linkage`) turns any orphaned entityId row into a named REVIEW on Books → Tx → Proof of the math; (4) Big Picture → Money renders a reconciliation note naming the orphaned $/mo and the fix path whenever the total and the entity cards can disagree; (5) the chunk-heal paints a visible "getting the latest version" notice before its reload so the skew recovery is never a bare blank screen. All proven-to-catch in vitest.
**Observability gap:** none of the money surfaces asserted they reconcile with each other; the entity-linkage check + the Money note close it.
**Cross-refs:** P14 (signed-in vs anonymous trust boundary — this is its third recurrence, one layer over), the 2026-06-11 provenance sweep (demo rows in cloud), the 2026-06-30 chunk-heal root-cause, DR-0076 (gates over claims).
