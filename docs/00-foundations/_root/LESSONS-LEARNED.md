...
- **P24 — When the principal is at a console and asks "anything else?", enumerate EVERY remaining step that console performs — and never call a step optional unless the fallback it relies on is verified.** The console trip is the scarcest resource in the loop; a truncated or softened checklist ("that can wait", "you're good") that turns out wrong costs a second trip and erodes trust faster than a longer honest list. "Optional" is itself a claim under DR-0076: it asserts the system is fine without the step — prove that, or say REQUIRED. (Extracted: 2026-07-05 — the poetech.us custom-domain click was called optional while Vercel was silently cap-frozen on an old build; Darrell left the laptop and had to be walked back in by phone.)
...
### 2026-07-05 — "Am I good at the laptop?" answered with an incomplete checklist + an unverified claim; the principal's console trip was wasted

**Trigger:** Mid Cloudflare-Pages cutover (~midnight), Darrell — standing IN the Cloudflare dashboard after fixing two secrets — asked "am I good... do you need anything else from me on the laptop?" The agent answered that the one remaining click (Custom domains → poetech.us → Activate) was "optional" and that "the site is serving fine on Vercel right now with tonight's build." He left. Twenty minutes later, on his phone, his screenshot showed poetech.us frozen at the OLD build (6E16800, Vercel daily-cap frozen) — the new features were only reachable via the domain click he'd just been told was optional. His response: "you should have been given me this information.... I was just in there!!!?"
**Detection path:** Darrell himself, from the mismatch between the promised features and the app on his phone.
**Detection delay:** ~20 minutes and one wasted console trip.
**Investigation steps:** Screenshot showed BUILD 6E16800 on poetech.us → cross-checked against the merge history: Vercel production never deployed the two newest merges (cap), while Cloudflare already had the newest build live at pages.dev; the domain flip was therefore the ONLY path to serve tonight's features.
**Root cause(s):** (1) Checklist truncation at the console: the agent knew the remaining step but framed it "optional / can wait" instead of enumerating it as the last required action while the principal was in the exact tool that performs it. (2) Unverified fallback claim: "Vercel is serving tonight's build" was asserted from the prior deploy's success, not verified — the newer merges were in fact cap-blocked, so the fallback the "optional" framing leaned on did not exist.
**What worked:** Owning the miss plainly; giving the phone-path for the same click; this entry.
**What didn't work:** Softening the ask to be kind to a tired principal — kindness that costs a second trip is not kindness.
**Principle(s) extracted:** P24 (below).
**Forward architectural fix:** When the principal is at a console and asks "anything else?", the agent enumerates EVERY remaining step that console can perform, ordered, each tagged REQUIRED-now vs deferrable — and may not tag a step deferrable unless the fallback state it relies on is VERIFIED (DR-0076: no claim without evidence). Pairs with the Drive-Don't-Delegate rule: his console time is the scarcest resource in the loop; spend it completely or not at all.
**Observability gap (if any):** The header freshness dot showed "● LATEST" on the stale build — the truth surface that should have exposed the frozen deploy instead endorsed it (queued as its own fix).
**Cross-refs:** feedback_desktop_paste_instructions (complete instructions when he's at the tool), Drive-Don't-Delegate (2026-05-23), DR-0076 (verification doctrine), P3 (deploy verification ≠ outcome verification).
