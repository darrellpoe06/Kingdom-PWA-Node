...
Carried today: the Torah pattern map (chips and prose), Study (plain layer, deep source, scripture line), Learn (stories and their verse line), the Scripture Library (cross-references), LessonFlow (every course and the living lessons), PracticeLearn (the teaching, the strands), and the Godhead study / Eternal Algorithms series. `BibleReader`'s chips open the reader because it *is* the reader. Pinned in `verse-chips.test.jsx` and `word-inline.test.jsx` (proven-to-catch: an `href`, invented text, a changed character of prose, or a surface reverting to a plain `<p>`/`<span>` each fails).

**Together, or one at a time — both; in order; and the reader is told (DR-0341, Darrell 2026-09-08).** `ShowTheWordToggle` (once, near the top of any surface that carries chips) flips the app-wide switch in `lib/show-the-word.js`: every verse on the page opens with one tap and closes with one; each chip still toggles on its own on top of it, and flipping the switch clears those individual choices. A page-wide open never nudges the screen. Opened verses read in **timeline-then-flow order** (`lib/scripture-order.js`: era band, then the Word's own arrangement, then chapter and verse — order, never a date; open placements named, never silently resolved). Beside the switch sits one plain sentence telling the reader all three. The shared matcher tells a book's NAME from an everyday word: "Job 19:25" is a verse, "the job 9:30" and "Job 9:30 am" are not.
