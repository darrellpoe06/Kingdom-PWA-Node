...
| R19 | Tiered-LLM orchestrator — local model (Ollama/4070) handles the routine; ESCALATES to vendor LLMs (Claude + Gemini) when it needs more power/better outcome; FALLS BACK to local when vendors refuse/can't (perpetual-fix resilience); runs on scheduled "vendor hours" so it works while Darrell works. HIGHEST-RISK CLASS (autonomous + spawns compute + spends real vendor $) → Tier C, three brakes MANDATORY (esp. a hard $/day budget + terminal condition per run), ships INACTIVE, never self-activates unattended | planned | DR-0056 + `2026-06-13-tiered-llm-orchestrator-spec.md`; DR-0001/0029/0037/0053; three-brakes (CLAUDE.md) | R4 runner (Darrell's infra values) + vendor API keys + Darrell greenlight + brake design | Darrell (governs) + Claude (builds) |
| R20 | Continuous fruit loop — Stage A discovery (fruit-scout built; Synthesizer-on-schedule next, inactive+brakes); Stage B execution auto-merges only additive-test class (Tier C, gated on R4 + governance policy) | planned | DR-0057; `2026-06-13-continuous-fruit-loop-spec.md`; `docs/governance/pre-authorized-policies.yaml` | R4 + Darrell greenlight per stage | Claude + Darrell |
| R21 | Recurring system review cadence — daily-review.yml (deterministic health/leverage scan) ships inactive; enable the cron deliberately; close W1-W4 (workflow-conformance) to unlock the 16 idle cron workflows | planned | DR-0058; `2026-06-13-rigorous-review-findings.md` | Darrell: enable cron when ready; W1-W4 work (R8) | Claude + Darrell |
