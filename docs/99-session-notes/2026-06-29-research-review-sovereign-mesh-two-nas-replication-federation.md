...
3. **The capability router already exists in embryo** — DR-0073 + `scripts/wake-router.mjs` pick a *worker by declared capability* behind the brakes. Generalize its "vendor" axis to a **node/capability** axis driven by a committed `nodes.json` registry: GPU jobs → the church GPU endpoint, storage/CPU jobs → the home DS1621xs. The **two church CUDA towers are set up as parallel sovereign *coding* workers** (Aider + Ollama + `qwen2.5-coder:14b`), so the tiers are: heavy/novel reasoning → vendor Claude; routine/bounded build + transcription + voice → the local towers in parallel. **Declared, not guessed.** Deterministic selection, no LLM in the dispatch decision. The honest limit — local coders are weaker on hard reasoning, so they take well-scoped tasks only; the vendor is reserved for the hard problems. (§2, §2e)
...
### 2e. The two church towers as parallel sovereign CODING workers (not just inference)
The two church CUDA towers (1× RTX 4070 each — the OBS/ProPresenter boxes) are set up as **networked local-coder workers**, not merely transcribe/voice endpoints. Each tower runs **Aider + Ollama + a coder model** — `qwen2.5-coder:14b`, already the Cage's `OLLAMA_CODER_MODEL` (~9 GB at Q5, fits one 4070 per DR-0012) — addressable across the Tailscale mesh. **Two towers = two parallel local coding workers**, alongside the home NAS. This is the sovereign destination of the existing Aider auto-commit lane (2026-06-01 sovereign-LLM-teams review; local-coding lane `local_7e8eed31` Hermes/OpenClaw), and it makes the GPU node an *active build worker*, not just a dumb-fast inference box.

**Tiered dispatch — deterministic-first + sovereign-LLM tiers** ([[project-deterministic-first-ai-only-necessary]], DR-0073):

| Tier | Work | Routed to |
|---|---|---|
| **Heavy / novel reasoning** | hard problems, cross-cutting design, ambiguous specs, anything where a plausible-but-wrong diff is expensive | **vendor Claude** (`ORCH_MODE=vendor-first`) |
| **Routine / bounded build** | well-scoped edits (rename, wire-a-known-pattern, mechanical refactor, test stub) | **local towers, in parallel** (least-loaded healthy tower; two towers fan out) |
| **Transcription + voice** | harvest no-caption transcription, voice clone | **local towers** (the §2d/§2c GPU jobs) |

So routine build + transcription + voice run **sovereign, free, in parallel across the two towers + home NAS**; only the hard problems spend vendor budget.

**The honest limit (binding, stated — not hidden — DR-0076):** local coder models are **materially weaker on hard reasoning** than the vendor. A local tower therefore takes **well-scoped tasks only** — a clear spec, a bounded blast radius, a verifiable result. The vendor is **reserved for the hard problems**; the router never hands novel/ambiguous reasoning to a 14B coder and calls it done. A local diff still passes the *same* gates (tests, the build, the legibility/consistency guards) before it is trusted — a sovereign worker earns trust the same way a vendor does (proven-to-catch). On a task that turns out to be hard, the correct local outcome is to **escalate to the vendor**, never to ship a confident-wrong diff.

**Routing + brakes shape:** a build job declares `requires: ["coder"]` + a `complexity` hint; routine → the least-loaded healthy tower (parallel); novel/over-threshold → vendor; no healthy tower → HOLD or vendor per the job class. Each tower's coder worker is a **passive, allowlisted endpoint**; the **three brakes live on the caller** (budget = vendor/compute ceiling, single-flight lock per tower, kill-switch), and **arming any tower's autonomous build loop is Tier C, his hand, never while traveling.** DR-0012 service-time preemption still holds: a tower carrying the Sunday/Wednesday encode/Proclaim load stops heavy model work during live service.
