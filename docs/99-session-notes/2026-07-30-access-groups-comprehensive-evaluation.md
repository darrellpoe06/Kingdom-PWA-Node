...
**5. DELIVERY-CONTEXT + THE DR-0108 CAPABILITY RE-SYNC — RUN, and it caught the review's own miss.** First draft of this dimension parked the NAS bearer rotation as "your hand" — matching the recorded bench (a human types it) WITHOUT running the DR-0108 challenge behind it. Darrell's correction: the `nas-bootstrap` remote-hands channel had driven that exact runner→tailnet→SSH-as-dpoe path for weeks, so the NAS-side rotation is **channel-drivable, not his hand**. Corrected here and made machinery: the rotation is now a one-dispatch tool (`nas-rotate-bearer.yml`, built this session); the standard's dimension 5 now REQUIRES the channel-challenge before any human-only classification; and `scripts/his-hand-guard.mjs` fails the build on a `re-review:` line that parks work as human-only without naming why no channel drives it (proven-to-catch). The genuinely-human tail is only reading the new secret + typing it onto the family devices (secret-onto-device, never a public log). This is what "re-sync to the LATEST product/processes/projects" must include: latest-of-what-the-team-can-build, not only latest-files.
...
7. **Token/device hygiene**: no expiry or per-device revocation for the bridge/review tokens; the NAS-side bearer still awaits rotation (now channel-drivable via nas-rotate-bearer.yml; only the device re-token is human); TS_AUTHKEY guidance permits a reusable key; the sudoers brake nas-bootstrap advertises is unversioned and unverifiable from the repo.
...
- **The review standard fixed the miss that produced a fake boundary**: COMPREHENSIVE-REVIEW-STANDARD dimension 5 now folds in the DR-0108 capability re-sync (name each team channel; a step is human-only ONLY if it is secret-onto-device / dashboard / bright line); `nas-rotate-bearer.yml` ships the rotation as a channel tool; `scripts/his-hand-guard.mjs` + its proven-to-catch test gate the class in CI so a bare "his hand" carry can never ship again.

## Carried (REV-0216 dated lines; watcher-swept; the lane executes)

Live smokes for 0082/0100 as matrix legs + wire `decideChildAction` into the real action paths (2026-08-01) · NAS bearer + review-token rotation — **channel-drivable** (nas-rotate-bearer.yml, built this session; the remote-hands channel does the NAS-side rotation over SSH), the only human tail is reading the new secret + typing it onto the family devices (secret-onto-device) (2026-08-01) · server-checked governor role to replace the email map, and grant-paths for `specialist` (2026-08-06) · assistant grant table + delegated-PM UI wiring, or an honest retirement of the unwired claims (2026-08-06) · denial telemetry aggregate + member-readable audit_log (rides DR-0242's 2026-09-15 line) · proxy rate-limit/allowlist decision + Vercel preview shutoff + GitHub secret-scanning/push-protection check — dashboard steps, listed for your sitting (2026-08-06) · DR-0253 promotion edge recorded and decided (2026-08-06).
