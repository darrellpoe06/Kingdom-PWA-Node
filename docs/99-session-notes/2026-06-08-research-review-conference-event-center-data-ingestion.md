...
3. **DATA-SOURCE INVENTORY — REAL findings (2026-06-09, §3.0), assumptions replaced with facts:** the church's **live ops do NOT live in the connected account.** **⭐ BIGGEST update: "Church Plus" runs on the church Synology NAS — now LOCATED on the network: `PoeTech` · `DS1621xs+` · `192.168.1.26` · DSM `7.3.2-86009`** — and this **appears to be the SAME Synology that is the PoeTech registry/sovereign substrate** (one device; the COLG node is already partly stood up — §3.1.6). **CONFIRMED on the NAS:** member data (the member system-of-record) **and** the monthly financial reports. **Giving's location is OPEN** (Church Plus? ConvertKit/Givelify? the reports? — don't assume). Web research → likely **PowerChurch Plus** (NAS-hosted `.DBF`), vs the cloud ChurchPlus.co; **package data model UNCONFIRMED until a DSM login (Darrell's).** **This re-ranks the path: NAS-local / sovereign is now PRIMARY; the Google Workspace grant drops to SECONDARY** (the genuinely-Google surfaces: the **TCOTLG Form** [Forms API], **ConvertKit**, **Zoom** — `@thechurchofthelivinggod.com`, primary `info@`). **P0a = Darrell reads Church Plus via a DSM login; P0b = the now-secondary Google grant.** Production ingestion = **n8n-on-NAS, sovereign, behind the Cage.**
...
|---|---|---|---|
| **⭐ "Church Plus" on the CHURCH NAS** (LOCATED 2026-06-09: Synology **`PoeTech` · `DS1621xs+` · `192.168.1.26` · DSM `7.3.2`**) | **PRIMARY AUTHORITATIVE SOURCE (sovereign path).** **CONFIRMED:** member data (system-of-record) + monthly financial reports on the NAS. **Giving = OPEN.** Same Synology as the PoeTech registry substrate (§3.1.6) | Read NAS-local via DSM/Synology APIs / `.DBF`/DB / file access at `192.168.1.26`; package data model **UNCONFIRMED pending a DSM login (Darrell's)** | **Sovereign / LAN-local — PRIMARY path; DSM login pending** |
...
6. **⭐ "Church Plus" on the church Synology NAS — the authoritative member system-of-record; NAS now LOCATED on the network (2026-06-09).** Darrell: *"we have Church Plus on the NAS at church."* **The NAS is now concretely identified** (read-only network discovery via Synology Web Assistant): **name `PoeTech` · model `DS1621xs+` · LAN IP `192.168.1.26` · DSM `7.3.2-86009` · status Ready**, on subnet `192.168.1.0/24`. **This is the same `DS1621xs` referenced as the registry/sovereign substrate across the PoeTech architecture and PR #8 — the "church NAS" and the PoeTech sovereign substrate appear to be ONE device** (see §3.1.6). **Confirmed data locations (Darrell, 2026-06-09):** (a) **Church Plus on this NAS hosts MEMBER data** — member rosters/profiles, the confirmed member system-of-record; (b) **the monthly financial reports ARE on this NAS** — confirmed (the §4c source); (c) **GIVING data location is UNKNOWN** — Darrell does not know whether giving lives in Church Plus, in ConvertKit/Givelify, or only in the financial reports → **leave giving's source an OPEN question, do not assume.** Web research (sources below): a NAS-hosted ChMS named "Church Plus" most likely = **PowerChurch Plus** (Windows-desktop app, data on the NAS share as **Visual FoxPro `.DBF`**, read NAS-local via ODBC/DBF parser/export; no public API), as distinct from the cloud **ChurchPlus.co** — **the exact package + data model is still UNCONFIRMED pending a DSM login (Darrell's, not the agent's).** **Do not fabricate its schema — see §3.1.6.**

**Architecture implications (baked into the design) — NAS-local is now the PRIMARY path:**

> **The data-access dependency is re-ranked (2026-06-09, updated with confirmed facts).** The church NAS is a **Synology `DS1621xs+` at `192.168.1.26` (DSM 7.3.2)** that **confirmedly hosts Church Plus member data + the monthly financial reports.** So **NAS-local ingestion (DSM/Synology APIs, direct DB/file access) is the PRIMARY recommended path; the Google Workspace grant drops to SECONDARY/supplementary** (for the genuinely-Google surfaces — the TCOTLG Form, ConvertKit, Zoom). **Giving's location stays an open question** (could be Church Plus, ConvertKit/Givelify, or the financial reports) — don't assume.

| Path | What it reaches | Sovereignty | Status |
|---|---|---|---|
| **Path A — NAS-local (Church Plus + financial reports), PRIMARY** | **Member rosters (confirmed)** + **monthly financial reports (confirmed)** + events/attendance — read **on the LAN** at `192.168.1.26` via DSM/Synology APIs / direct `.DBF`/DB / file access (§3.1.6). **Giving = OPEN (maybe here, maybe ConvertKit/Givelify, maybe the reports).** | **Fully sovereign, LAN-local, no vendor cloud, no Google grant** | **NAS located + data partly confirmed; package data-model UNCONFIRMED pending a DSM login (Darrell's)** |
...
- **Recommendation:** **prefer Path A (NAS-local) as the member + financial system-of-record** — it is confirmed to hold member data + the monthly reports, it is fully sovereign, and it **strengthens "don't start from scratch"** (the church already has a populated ChMS on the same Synology that is the PoeTech substrate). Use Path B only for the genuinely-Google surfaces (the Form, ConvertKit, Zoom). **Resolve giving's location** before wiring giving ingestion. The remaining unknown is the Church Plus **package + data model** — read it via a DSM login (Darrell's), then design the §3.1.6 adapter; **schema stays UNCONFIRMED until then.**
...
**⭐ The NAS is now LOCATED on the network (read-only discovery via Synology Web Assistant, 2026-06-09):**

| Field | Value |
|---|---|
| **Name** | `PoeTech` |
| **Model** | **Synology `DS1621xs+`** |
| **LAN IP** | **`192.168.1.26`** (subnet `192.168.1.0/24`) |
| **DSM version** | **`7.3.2-86009`** |
| **Status** | Ready |

- **The NAS-local ingestion path is now CONCRETE:** DSM 7.3.2 at `192.168.1.26` — reachable on the church LAN via **Synology DSM APIs / direct local DB / file access** to the Church Plus data + the monthly financial reports. No internet egress, no vendor cloud.
- **⭐ Same device as the PoeTech sovereign substrate.** This `DS1621xs+` at `192.168.1.26` is the **same `DS1621xs` named across the PoeTech architecture and PR #8 as the registry / sovereign substrate** (Postgres+pgvector, batch inference, the n8n runner — and the `192.168.1.26` already in CLAUDE.md / the dispatch-status convention). **So the "church NAS" and the "PoeTech NAS" appear to be ONE physical device** — the COLG sovereign node is not just *planned*, a Synology is already on-site doing double duty. (Worth Darrell's explicit confirmation that these are indeed the same box.)
- **Relation to the PR #8 COLG sovereign node:** this strongly implies the COLG node is **already partially stood up** — the church-entity instance, the Path-A ingestion, and the existing PoeTech registry can co-reside here. It is plausibly the same network where the **`infra/ai-orchestrator/` actions still stubbed "pending real UniFi/Netgate/VLAN values"** will resolve; locating this NAS is a step toward mapping that network.

**Confirmed data locations on this NAS (Darrell, 2026-06-09):**
- ✅ **Member data — CONFIRMED on the NAS in Church Plus** (member rosters/profiles; the member **system-of-record**).
- ✅ **Monthly financial reports — CONFIRMED on the NAS** (the §4c source for annual results / financial reporting).
...
- Wrapped as an **`ingestion_sources` adapter** (`source_system='church_plus'`) like any other (§3.1.5): idempotent upsert keyed by the ChMS record id, watermark on a modified-timestamp/export-date, Cage-allowlisted read-only action (`fetch_church_plus.sh`) against `192.168.1.26`, hash-chained ledger row per run, four brakes, ships inactive. Feeds `parishioners`/`external_users` (**member rosters — confirmed**), `service_offerings`/annual results (**monthly financial reports — confirmed**, §4c; **giving = open**), `events`, `attendance_records`.
...
**Relation to the PR #8 COLG sovereign-node architecture:** the church NAS is **existing on-site infrastructure that is now located** (`DS1621xs+` @ `192.168.1.26`, above) — and **appears to be the same Synology that already serves as the PoeTech registry/sovereign substrate.** So the COLG sovereign node is **already partially stood up**: the church-entity instance, the Path-A ingestion, and the existing PoeTech registry co-reside on one box. Plausibly the same network where the **`infra/ai-orchestrator/` actions still stubbed "pending real UniFi/Netgate/VLAN values"** will resolve. (Keeps the church-entity data **on church hardware** — maximally aligned with `AI-FOUNDATION-INTERNAL-OPERATIONS` + sovereignty.)
...
- **Upstream source (CONFIRMED 2026-06-09): the church's MONTHLY FINANCIAL REPORTS are stored ON THE NAS** (the Synology `DS1621xs+` @ `192.168.1.26`, §3.1.6) — confirmed by Darrell. **This fills the earlier "annual results / financials = NOT FOUND" gap and reinforces the NAS-local (Path A) ingestion.** **Annual results are COMPUTED / rolled up from the monthly reports — never hand-keyed.** Exact file format is **UNCONFIRMED** (a Church Plus report? a Sheet? PDFs on the share?) — **do not fabricate the schema;** read NAS-local via the §3.1.6 adapter once the format is known.
...
|---|---|---|---|
| **⭐ P0a — Read Church Plus on the NAS (PRIMARY, §3.1.6)** | NAS located: `DS1621xs+` @ `192.168.1.26` (DSM 7.3.2), hosting member data + monthly reports (confirmed). **Darrell does a DSM login** to read the Church Plus package + data model + resolve where giving lives; then design the NAS-local adapter. | **Darrell DSM login** (agent will not); LAN access | **immediate — the primary path** |
...
- **⭐ Church Plus on the church Synology NAS — LOCATED + partly confirmed (2026-06-09).** NAS identity: **`PoeTech` · `DS1621xs+` · `192.168.1.26` · DSM `7.3.2-86009`** (read-only Web-Assistant discovery) — **the same Synology that is the PoeTech registry/sovereign substrate** (one device; §3.1.6). **CONFIRMED on it:** member data (rosters — the member system-of-record) **and** the monthly financial reports. Web research → most likely **PowerChurch Plus** (NAS-hosted `.DBF`), distinct from cloud ChurchPlus.co. **Now the PRIMARY authoritative source on the sovereign path** — supersedes the Google grant for members + financials.

**STILL UNKNOWN — what only a DSM login can answer (Darrell logs in; the agent will NOT) + the open giving question:**
- **⭐ Church Plus package + data model (highest leverage; §3.1.6):** the exact product/version and **its DB/file layout** — read it via a DSM login at `192.168.1.26`, then design the adapter. **Schema NOT fabricated until then.**
