...
3. **DATA-SOURCE INVENTORY — REAL findings (2026-06-09, §3.0), assumptions replaced with facts:** the church's **live ops do NOT live in the connected account.** **⭐ BIGGEST update: "Church Plus" runs on a NAS PHYSICALLY AT THE CHURCH** — likely the **authoritative system-of-record** (web research → most likely **PowerChurch Plus**, a NAS-hosted desktop ChMS w/ Visual FoxPro `.DBF` data, holding rosters + **giving + monthly financial reports** + events + attendance; **UNCONFIRMED** vs the cloud ChurchPlus.co — §3.1.6). **This re-ranks the data path:** if confirmed, ingestion goes **NAS-local / sovereign (no Google grant)** and the Google grant becomes **secondary.** The other real sources: a **separate Workspace `@thechurchofthelivinggod.com`** (primary `info@`; + the **TCOTLG Registration Google Form** [Forms API] + the **Gwin Home-going sheet** = seedable now), **ConvertKit** (members + schedule + Bible-Trivia), **Zoom** (National Assembly). **P0a = confirm Church Plus (3 questions to Darrell); P0b = the now-secondary Google grant** for the genuinely-Google surfaces. Production ingestion = **n8n-on-NAS, sovereign, behind the Cage.**
...
|---|---|---|---|
| **⭐ "Church Plus" on the CHURCH NAS** (on-site, 2026-06-09 — *"we have Church Plus on the NAS at church"*) | **LIKELY THE AUTHORITATIVE SYSTEM-OF-RECORD** — may already hold member rosters, giving, **financial reports**, events, conference data we'd otherwise wait on the Google grant for | On-site Synology NAS at 312 E. Bradley Ave; product/version **UNCONFIRMED** (likely **PowerChurch Plus** desktop ChMS w/ data on the NAS share — §3.1.6) | **Sovereign / LAN-local — pending Darrell's detail on what it is + how to reach it** |
...
6. **⭐ "Church Plus" on the church NAS (NEW, 2026-06-09) — likely the authoritative system-of-record, UNCONFIRMED.** Darrell: *"we have Church Plus on the NAS at church."* Web research (sources below) finds the NAS-at-church detail most likely matches **PowerChurch Plus** — a **Windows-desktop church-management system whose shared data files are hosted on a NAS** (PowerChurch KB 487), stored as **Visual FoxPro `.DBF` files** readable via the **FoxPro ODBC driver or a DBF parser** (reads work; in-place ODBC updates are unreliable — fine for read-only ingestion). It typically holds **membership, contributions/giving, accounting + financial reports, event scheduling, attendance.** **There is also a cloud SaaS named ChurchPlus.co — a DIFFERENT product (vendor cloud, not NAS-hostable)** — so the exact product **must be confirmed.** No public API is documented for the desktop product → ingestion = **read the `.DBF` files / ODBC / built-in export, NAS-local.** **Conference *registration* specifically is UNCERTAIN in the desktop product** (event *scheduling* is confirmed). **Do not fabricate its schema — see §3.1.6; UNCONFIRMED pending Darrell.**

**Architecture implications (baked into the design) — TWO ingestion paths, re-ranked by the Church Plus finding:**

> **The data-access dependency is re-ranked (2026-06-09).** If Church Plus on the church NAS is the real system-of-record, **ingestion may go NAS-local (sovereign, DB/export-direct) and the Google Workspace grant becomes SECONDARY** (for the few surfaces that genuinely live in Google — the TCOTLG Form, ConvertKit, Zoom). Both paths below; recommendation favors NAS-local **if** Church Plus is confirmed to hold the data.

| Path | What it reaches | Sovereignty | Status |
|---|---|---|---|
| **Path A — NAS-local (Church Plus), PREFERRED if confirmed** | Member rosters, **giving + monthly financial reports**, events, attendance — read **on the LAN** from the `.DBF` files / ODBC / built-in export on the church NAS (§3.1.6) | **Fully sovereign, LAN-local, no vendor cloud, no Google grant** | **UNCONFIRMED — pending Darrell on product/version + data + reach** |
| **Path B — Google Workspace grant (now SECONDARY)** | The surfaces that really are in Google: the **TCOTLG Form** (Forms API), Drive/Sheets; + the **ConvertKit** + **Zoom** external adapters | Sovereign-runner (n8n) hitting external APIs | Grant still needed for *these* surfaces (below) |

- **Path B P0 (unchanged, now scoped to the Google-resident surfaces) = a read-only service-account / domain-wide-delegation grant on `thechurchofthelivinggod.com` — targeted at the PRIMARY/ADMIN account `info@thechurchofthelivinggod.com` — + the Google Forms API scope.** `info@` is the church's main Workspace account (and the confirmed ConvertKit sender). **Production ingestion = n8n-on-NAS hitting the Workspace + Forms APIs directly** (sovereign, API-not-browser, per `AI-FOUNDATION-INTERNAL-OPERATIONS`) — credentials in the n8n credential store, read-only scopes, behind the Cage.
- **Recommendation:** **confirm Church Plus first** (one question to Darrell — §3.1.6). If it holds rosters/giving/financials/events, **prefer Path A (NAS-local) as the system-of-record** and use Path B only for the genuinely-Google surfaces (the Form, ConvertKit, Zoom). This is the more sovereign path and **strengthens "don't start from scratch"** — the church may already have a populated ChMS.
...
Darrell: *"we have Church Plus on the NAS at church."* This is potentially the **single most important source in this whole inventory** — the church's own ChMS, on the church's own hardware. Treated as **UNCONFIRMED pending Darrell** on three points; no schema is fabricated.
...
- Wrapped as an **`ingestion_sources` adapter** (`source_system='church_plus'`) like any other (§3.1.5): idempotent upsert keyed by the ChMS record id, watermark on a modified-timestamp/export-date, Cage-allowlisted read-only action (`fetch_church_plus.sh`), hash-chained ledger row per run, four brakes, ships inactive. Feeds `parishioners`/`external_users` (rosters), `service_offerings`/annual results (**giving + monthly financial reports**, §4c), `events`, `attendance_records`.
...
**Relation to the PR #8 COLG sovereign-node architecture:** the church NAS is **existing on-site infrastructure** — the COLG sovereign node may already be **partially stood up** (a NAS is physically there running the ChMS). This is the natural host for the church-entity instance/Path A ingestion, and plausibly the same network where the **`infra/ai-orchestrator/` network actions still stubbed "pending real UniFi/Netgate/VLAN values"** will live. Confirming Church Plus also helps map that network. (Keeps the church-entity data **on church hardware** — maximally aligned with `AI-FOUNDATION-INTERNAL-OPERATIONS` + sovereignty.)

**UNCONFIRMED — three questions for Darrell (do not fabricate answers):**
1. **What is it exactly?** PowerChurch Plus (desktop, `.DBF` on the NAS) or the cloud ChurchPlus.co or another product/version?
2. **What data does it hold?** Rosters? Giving + the monthly financial reports? Events? Conference registrations? Attendance?
3. **How do we reach it?** A NAS file-share path to the data folder + read-only credentials? ODBC? Or only manual exports? (And the NAS model / network — ties to the sovereign node.)

> Until confirmed, Church Plus is carried as a **high-probability authoritative source on the sovereign path**, not a built dependency. If confirmed, it likely **supersedes the Google grant** for rosters/giving/financials/events and becomes the church-entity system-of-record.
...
- **Upstream source (NEW, 2026-06-09): the church's MONTHLY FINANCIAL REPORTS** — the church produces **monthly financial reports**, likely in **Church Plus / on the church NAS** (§3.1.6). **This fills the earlier "annual results / giving = NOT FOUND" gap.** **Annual results are COMPUTED / rolled up from the monthly reports — never hand-keyed.** Exact location/format is **UNCONFIRMED** pending Darrell (in Church Plus? a Sheet? PDFs?) — **do not fabricate the schema;** ingest via the §3.1.6 Church Plus adapter (NAS-local, preferred) or the appropriate adapter once the format is known.
- **In-app source of truth:** the **transactional tables** — `service_offerings` (giving, incl. `service_kind='conference'`), `attendance_records` (new), `volunteer_hours`, `event_participants`, `ministry_signups`, `events` — populated **from the monthly reports / Church Plus.** Annual results are **derived.**
...
|---|---|---|---|
| **⭐ P0a — Confirm Church Plus on the NAS (FIRST, §3.1.6)** | One question to Darrell: what is it / what data / how to reach it. If it's the ChMS system-of-record, **the NAS-local path (Path A) likely supersedes the Google grant** for rosters/giving/financials/events. | **Darrell answer** (3 questions) | **immediate — gates the path choice** |
| **P0b — Google grant + adapters (now SECONDARY, scoped to Google-resident surfaces)** | Read-only **service-account grant on `@thechurchofthelivinggod.com` via `info@` + Forms API scope**, + **ConvertKit + Zoom read-only API keys**. | **Darrell/church-admin (`info@`) grant**; n8n creds | **~2–4 days** (gated on grant) |
...
- **⭐ "Church Plus" on the church NAS (2026-06-09)** — a church-management system on a Synology NAS **physically at the church**; web research (sources below) makes **PowerChurch Plus** the most likely match (Windows-desktop ChMS, data on the NAS as Visual FoxPro `.DBF`, readable NAS-local; holds membership + giving + financial reports + events + attendance). **Likely the authoritative system-of-record on the sovereign path** — and likely the home of the monthly financial reports (§4c). **Product/version + reach UNCONFIRMED (see below).**

**STILL UNKNOWN / NEEDS DARRELL OR THE GRANT (precise — not invented):**
- **⭐ Church Plus — three UNCONFIRMED questions (highest leverage; §3.1.6):** (a) **what it is** (PowerChurch Plus desktop on the NAS vs the cloud ChurchPlus.co vs another product/version); (b) **what data it holds** (rosters? giving + monthly financial reports? events? conference registrations? attendance?); (c) **how to reach it** (NAS file-share path to the `.DBF` folder + read-only creds? ODBC? manual export only?). **If it holds the data, it likely SUPERSEDES the Google grant for rosters/giving/financials/events** (sovereign, NAS-local). **Schema NOT fabricated.**
- **Monthly financial reports — exact location/format UNCONFIRMED** (in Church Plus? a Sheet? PDFs?) — they are the **source for annual-results + giving (§4c)**, computed not hand-keyed; **ISO-2, staff-gated, never public, never in seed.**
- **Speaker/participant rosters + bios** — not in the connected account → **Church Plus (likely) or the `@thechurchofthelivinggod.com` Workspace (grant).**
