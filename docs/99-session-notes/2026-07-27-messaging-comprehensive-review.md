...
| **1c — Pictures in messages** (Darrell 2026-07-27: "send pictures etc") | Supabase Storage bucket `message-media` (instance-scoped path, RLS mirrors the thread predicates — a photo is readable exactly by whoever can read its message); composer 📎 via UiIcon + file input (accept image/*, client-side downscale ≤1600px, size cap with honest error); `media_path` column on both message tables; inline thumbnail render with tap-to-full; DM media rides the SAME E2EE posture honestly labeled (encrypt-before-upload is the Tier-C follow-up with DM body E2EE parity — until then media is RLS-private like group bodies, stated in UI) | ≈ #1057-class (migration + composer + render) |
...
| **3 — P2 contact cards + ADD CONTACT (Darrell 2026-07-27: "messages need a add contact etc so we can invite and add contacts at the same time")** | `member_contacts` (self-writable, visibility member-chooses); directory reads roster+cards; and an **Add contact** button ON the Messages Direct tab: enter a name + email once → sends the in-app invite AND creates the pending contact card in the same move — the invited person appears immediately as a visible pending chip (closes the owner's empty-roster confusion through the front door) | ≈ 0096-class migration + one panel |
