...
### OPEN-8 · Receipt OCR — auto-read the merchant + total off the photo
- **Unblocks:** the hand-typing on every receipt (2026-07-05 Darrell typed `86.68` + `Aspen Tap House` by hand). Read them off the photo so capture is one tap. Feeds the tip-inclusive matcher that shipped this session.
- **The fork:** deterministic-first on the **NAS ops-runner** (DR-0088 lane, sovereign, no per-read vendor cost) or a vision-LLM call (faster to build, needs a key + egress + VISION-FAIRNESS review)?
- **Track:** NAS ops-runner (I can't reach the LAN). **Tier B** for the NAS deterministic path; **Tier C** if it calls a vision vendor.
- **My recommendation:** NAS deterministic-first (matches AI-FOUNDATION-INTERNAL-OPERATIONS + DR-0090's stated Phase-2 plan); the total feeds `suggestMatches` directly, so accuracy compounds.
- `DECISION:` _____

### OPEN-9 · Synced pending receipt pool (snap on phone, match on laptop)
- **Unblocks:** cross-device pairing. Today the "waiting to match" pool is per-device `localStorage` — a receipt Christina snaps on her phone can't be matched from the laptop. This is DR-0090's own **`re-review: 2026-08-01`** item.
- **The fork:** a new synced `pending_receipts` table (family-scoped, same isolation as the financial rows) vs. keeping it device-local until it actually hurts.
- **Track:** app + migration lane. **Tier B** (schema; additive; family-scoped).
- **My recommendation:** wire it when the second device actually needs it (Christina's phone → laptop) — that's the trigger DR-0090 named; not before, to avoid schema we don't yet use.
- `DECISION:` _____

### OPEN-10 · Multi-page receipt (itemized slip + signature/tip copy)
- **Unblocks:** the real shape of a restaurant receipt — the 2026-07-05 photo is **two slips** (itemized total + the signature copy carrying the handwritten tip). Only one image attaches today, so the tip evidence is lost.
- **The fork:** extend the receipt shape from a single `src` to `pages[]` (needs the sync `toRow`/`fromRow` + update-patch mapping to carry an array on the money row), or keep one photo and let the user pick which slip.
- **Track:** app + transactions-sync. **Tier B** (data-shape change on a money row; additive, migration-compatible).
- **My recommendation:** `pages[]` — the tip copy IS the evidence for the +tip charge the matcher now pairs; capturing only the pre-tip slip undercuts the paper trail DR-0090 exists to build.
- `DECISION:` _____
...
- **2026-06-17 · Dark-mode contrast bug fixed + the blind spot that hid it closed — `fix/dark-mode-text-contrast` (PR #257).** Reported bug: BLACK/dark text on the DARK (midnight) background in cockpit surfaces — unreadable. Root cause: `CommandServeCenter.jsx` and `BuildBoard.jsx` set accent text via **inline** `style={{ color: '#…' }}`, which bypasses the per-`[data-theme]` remap, so midnight flips the surface dark while the inline text stays dark; the blue accent `#2A5A8E` also had **no midnight remap** at all (2.84:1 on black). The AA contrast guard never caught it because it only checked body-text tokens, never accents, and never inline colors. **Fix (Tier A, ships fast):** (1) converted every inline accent color in the two files to themeable `text-/bg-/border-[#…]` classes; (2) added the missing midnight remaps (`#2A5A8E`→`#7FB3F0`, `#5A5751` fill/border); (3) **extended `contrast-guard.mjs`** to check accents PER-THEME including midnight (proven-to-catch: it now flags a missing blue remap at 2.84:1) and added an **inline-color scanner** (guarded cockpit files hard-fail on any inline color; other component files WARN). 15/15 guard tests green; verified AA in BOTH light and midnight.
  ...
  - **Follow-up — PROMOTED 2026-07-10 (the 07-15 item, done early):** `Imported.jsx` (every literal AND ternary inline color converted to themeable classes) and `TeachMode.jsx` (already cleaned by its owning lane) are now in the guarded set and hard-fail on any inline color; midnight remaps added for the five tokens the conversion exposed (`#216E39`, `#9B2C2C`, `#2F6B3A`, `#946A00`, `#16A34A`). Bug-class instances (inline TEXT color over a themed background) also killed in `Bookstore.jsx`, `Forecast.jsx`, `CreationWorkspace.jsx`.
  - **Remainder (tracked, not silent):** `Bookstore/Forecast/CreationWorkspace` still carry palette-object inline styling (`P.ink` etc., ~80 sites) — a whole-file conversion lane; `Presenter.jsx` (26) + `Pulpit.jsx` (5) are self-contained fixed overlays (fg+bg pinned together — not the dark-on-dark class) that may deserve a scanner category of their own instead of conversion. Promote or categorize as each owning lane settles. **`re-review: 2026-07-24`.**
...
- **2026-07-05 · Receipts capture + matching hardened (Tier A) — `claude/poetech-receipt-upload-9bcxib`.** Reported live by Darrell (4 screenshots): the Add-a-receipt overlay showed a chosen filename but the Save button stayed stuck on "ADD THE PHOTO FIRST" — receipts couldn't be uploaded. **Two ships:** (1) **PR #604 (merged 20:04Z, ~2m33s push→merge)** — the file input never reset its value, so re-picking the SAME photo fired no `change` event and Save never unlocked; fixed by resetting the input value on every change, clearing `src` per read, rejecting an empty decode, and adding a standing hint. (2) **This session** — the matcher's exact-cent rule NEVER paired a tipped receipt to its settled charge (paper $72.60 vs VISA $86.68 with the handwritten tip — the exact Aspen receipt DR-0090 was built around): added a **bounded, one-directional tip tolerance** (`TIP_TOLERANCE_PCT = 0.30`; charge must be ≥ paper and ≤ +30%), **merchant-name ranking** (`merchantOverlap`, rank-only — never widens/narrows candidates), and **match-kind labels** (`exact` / `+ tip?` / `date only — verify`) so an inexact pairing is flagged for the human to confirm (VERIFICATION-DOCTRINE). Suggestion-layer + human-confirmed + proven-to-catch on the verbatim Aspen numbers; receipts.test.js 13/13, full suite 4539/4539, lint + per-theme contrast guard green. Full write-up: `docs/99-session-notes/2026-07-05-receipts-algorithm-review.md`. **Escalated for your call → OPEN-8 (OCR), OPEN-9 (synced pool), OPEN-10 (multi-page receipt).**
  - **Perpetual-improvement (deferred, dated — DR-0075):** G4 (symmetric ±4-day window — a charge never settles before the paper, so pre-dating is noise) and G3 (pool dedupe) both **`re-review: 2026-08-01`**, folded into OPEN-9 so the matching-semantics change lands with the synced-pool work rather than as a standalone tweak.
