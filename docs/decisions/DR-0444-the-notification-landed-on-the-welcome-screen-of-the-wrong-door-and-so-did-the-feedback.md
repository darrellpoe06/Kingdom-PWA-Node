...
- **The offline fallback is still PoeTech's shell** for every door: `sw.js` falls back to `BASE + '/index.html'`, so a church member offline on `/lovecorner/app/` gets the family shell's cached page. ~~Real but narrow.~~ **CORRECTED THE SAME DAY, by measurement rather than by estimate — and it is neither narrow nor unaddressed.**

  **PR #1405, "The church's door gets its own offline shell — and its own witness", already fixes exactly this**, and it has been open since **2026-08-30**. Its report is an actual outage: Darrell's phone on `https://poetech.us/lovecorner/app/?view=church&lovecorner=1` returning **`ERR_FAILED`** while `site-health.yml` simultaneously reported "UP. Fresh." Its trace names the mechanism my note under-rated: for a `/lovecorner/app/` navigation, `caches.match(BASE + '/index.html')` resolves to the WRONG app's shell when that entry exists and to **`undefined`** when it does not — and `respondWith(undefined)` IS a network error, which Chrome renders as `ERR_FAILED`. On 4G one transient failure is enough. So this is not "offline navigation only": it is a dead front door on a flaky connection.

  Measured against `main@a2078515` on 2026-09-16, the defect is **live**: `sw.js:70` still reads `caches.match(BASE + '/index.html')`, the worker carries no `SCOPE_SHELLS`, and `site-health.yml` probes **none** of the four door faces — so the door the congregation taps still has no witness of its own, which is the second half of what #1405 fixes.

  **It also now overlaps this DR.** Both changes need a list of doors inside `sw.js` — #1405 for the offline shell (`SCOPE_SHELLS`), this DR for the notification-click door preference (`DOOR_PATHS`) — and both should resolve through ONE list, with `app-doors.js` as its source. Whoever lands second must unify them rather than carry two.

  **Recommendation (the author is Darrell, and it is his branch, so this is put to him rather than pushed):** rebase #1405 onto current `main` and let it merge — the defect it fixes is live, its proof is already written (5 worker-level tests, one of which reproduces the `respondWith(undefined)` mechanism itself), and its base is 17 days stale. Its own "honest limits" section is still accurate: it is the structural fix, not an on-device confirmation. **re-review: 2026-09-23**, and this item is now tracked as "#1405 is waiting", never as undiscovered work.
