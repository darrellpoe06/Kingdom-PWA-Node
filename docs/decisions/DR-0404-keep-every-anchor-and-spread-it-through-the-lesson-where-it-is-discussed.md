...
- After the write: of 591 anchor references in L141–L152, named 43 → **406**; quoted-but-unnamed 241 → **0** in every field (range-aware: one verse cited beside a two-verse quote names it, which is the originals' own practice); neither → **185**, owed and dated; corruptions 3 → 0. Lint clean; the 12 per-lesson verse gates (L141's 39/39 after the repair), the reading-band gates, the adversary-case gate and the new naming gate (7/7) green on the written corpus.
- The diff is reviewable line by line: every `+` is an appended `(Ref)`, an extended label, or `'s` restored.

## Limits, stated

- **185 verses are still owed** (plus 3 pre-existing in L54/L81), and this record does not pretend otherwise. First lesson for the live test: **L151 The Tongue** (93 owed, the worst, and the one on his screen). **re-review: 2026-09-16** — L151 authored and seen live, or a why.
