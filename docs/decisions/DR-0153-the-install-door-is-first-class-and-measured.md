...
2. **His blank screen was a real outage class, not user error:** during each deploy, poetech.us serves the new shell before its assets are reachable — missing chunks answer as the SPA fallback HTML and zero JavaScript runs (~7–12 min windows, twice measured 2026-07-10). Diagnosed with evidence on incident #722; the durable-fix decision (asset-404 vs zone cache purge) carries `re-review: 2026-07-12` [DONE 2026-07-10 by DR-0155 — asset guard live at app/functions/poetech-app/assets/[[path]].js, 16 proven-to-catch assertions in asset-guard.test.js green, propagation gate in deploy-cloudflare-pages.yml; verified 2026-09-08].
