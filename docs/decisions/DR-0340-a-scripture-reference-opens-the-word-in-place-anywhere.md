...
Measured 2026-09-08 before writing (DR-0314): five components map a `refs` array to markup; nineteen render `whitespace-pre-wrap` prose. Wired in this change: the Torah pattern map (chips + `shows` / confession / reticence prose), Study (plain layer, deep source, scripture line), Learn (a story and its verse line), the Scripture Library (cross-reference chips). `EternalAlgorithmsStudy` already quotes its verses inline; `BibleReader`'s chips open the reader because it *is* the reader. ~~**re-review: 2026-09-22** — walk the remaining prose surfaces (LessonFlow, PracticeLearn, the living-lessons bodies, the Godhead study) onto `WordInline`~~ **DONE 2026-09-09** (Darrell: "do the LessonFlow, PracticeLearn and Godhead study now... don't wait"): `LessonFlow.jsx` (the shared Learn engine every course and the living lessons ride — both blurbs, plus `ShowTheWordToggle` in both views), `PracticeLearn.jsx` (big idea, every teaching paragraph, quiz explanations, course summary, the four strands), `EternalAlgorithmsStudy.jsx` (the Godhead study's outcome / then / psyche, the series' plain and deep layers, the forge's three layers, the covenant review, the intertwine, the study intro; the toggle under the Godhead banner and beside the study intro). Pinned in `word-inline.test.jsx` and `show-the-word.test.jsx`. The source-scan gate question stays open with its reason: measured 2026-09-09, a regex cannot tell a teaching paragraph from a label without noise — **re-review: 2026-10-09**.
