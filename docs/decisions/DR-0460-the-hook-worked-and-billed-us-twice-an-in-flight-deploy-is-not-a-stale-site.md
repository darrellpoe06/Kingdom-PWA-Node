...
**OBSERVED 2026-09-17, 20:00 UTC — the merge that carried this fix did NOT test it, exactly as the paragraph above refused to predict.** Measured: PR #1651 squash-merged as `f4f833f8`; `deploy-freshness` run #1318 fired on `workflow_run` at 19:59:21 and **ran the PRE-merge definition of the file** — its inlined script, read back from the run log, carries no in-flight query and no third return code at all. It reported `FRESH` on look 1 (main still `003aaf22`), then `STALE` on look 2 and dispatched, exiting on the dispatch. Result for that merge: **exactly one** `Deploy (Cloudflare Pages)` run for `f4f833f8` (#1159, 20:00:13, success), against **two** for the previous merge (#1157 and #1158 on `003aaf22`). So the outcome was right and **the gate is still unexercised** — one deploy happened because only one dispatcher fired, not because anything was suppressed. The new definition is on `main` now, so the NEXT merge is the first that can run it, and the proof to look for is unchanged: one deploy run for the new tip, and a `deploy-freshness` run that prints `ALREADY IN FLIGHT`. And the single build IS attributable, measured rather than assumed: auto-merge run #4386's deploy-heal job printed `20 armed PR(s) may merge momentarily — polling up to 12 minutes` at 19:46:30 and `Poll window closed` at **19:58:57**, seconds before the merge landed — so it never dispatched at all, and the DR-0458 `workflow_run` hook is the only reason `f4f833f8` was served. That is the DR-0458 gap firing a second time, in the same shape (the poll expiring just short of the merge), and it is now twice-proven rather than once. `re-review: 2026-09-18` stands.

**PROVEN 2026-09-17, 20:38 UTC, ON THE NEXT MERGE — and this closes the section above.** PR #1652 squash-merged as `57033d99`. `deploy-freshness` run #1319 fired on `workflow_run` at 20:37:16, and **the definition it ran is the new one**: the script GitHub echoes into the log at the start of the step carries the in-flight query, the `ALREADY IN FLIGHT` branch and the `rc -eq 2` arm, which run #1318 an hour earlier did not. It looked twice while main still read `f4f833f8` (`FRESH`), saw `57033d99` on look 3, queried for a deploy run on that exact tip, found none, and dispatched **one** — `Deploy (Cloudflare Pages)` #1160, 20:38:57, green at 20:41:26, and the only run for that tip. Against two runs for `003aaf22`, that is the regression closed: **one deploy per merge, measured.** What is still unexercised in production is the suppression branch itself — `ALREADY IN FLIGHT` has not yet had to print, because auto-merge's heal did not dispatch first on either of these two merges. That branch is proven in CI (the break that narrows the query to successes only is caught) and it will print the first time auto-merge's poll wins the race; nothing further is pending here.
