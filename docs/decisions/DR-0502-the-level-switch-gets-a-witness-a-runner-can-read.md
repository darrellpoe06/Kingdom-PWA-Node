...
**The answer itself is not in hand, and one more constraint was measured while trying to get it.** A `workflow_dispatch` workflow cannot be dispatched until it exists on the **default branch** — dispatching `level-witness.yml` on its own branch returns `404 Not Found`. So this witness fires either after the PR merges (dispatched on `main`) or on its daily 12:41 UTC schedule, and the first finding arrives on the incident ledger then — or the run passes, which is itself the answer. Recorded because the same 404 will confuse the next person who builds a dispatch-only workflow on a branch and expects to try it there. Saying "the reader is fine" or "the defect is real" before reading that would be the same over-claim this session already had to correct once (DR-0497's count). **re-review: 2026-09-19.**
