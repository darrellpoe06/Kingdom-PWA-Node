...
scope:
  ...
  - infra/supabase/migrations-auto/0234-every-intake-is-carried-to-an-outcome.sql + infra/supabase/tests/0234-intake-outcome-smoke.sql + the rls-isolation leg
...
10. **Low-hanging fruit is fixed by the system, through the lane, with the full brake set.** The flow:
   ...
   - It enqueues the low-hanging ones in `intake_fix_queue` (migration 0234: stewards read it; nobody signed in inserts).
...
- `app/src/__tests__/intake-sync.test.js`: one id per note; the category written at birth; a reply carries reply_to; the sender's own notes read back with outcomes; "fixed" writes what changed and when. The rest still holds on a database without 0234: a different missing column is not mistaken for these.
...
- `infra/supabase/tests/0234-intake-outcome-smoke.sql` in the rls-isolation matrix (leg `intake-outcome`):
