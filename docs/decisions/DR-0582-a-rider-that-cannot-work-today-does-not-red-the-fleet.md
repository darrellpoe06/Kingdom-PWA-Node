...
**Measured once more (run 35883853346, 15:47Z):** the line printed `hosted key in agent.env: opaque` — the value is not a legacy JWT at all, so it came from the NEW API Keys tab, not the Legacy one the screenshot showed. Supabase's new keys carry their role in the prefix: `sb_publishable_` is the anon role, `sb_secret_` is service_role, and the gateway translates the `apikey` header to the matching role (docs: self-hosted-auth-keys, api-keys). A publishable key explains "Bucket not found" exactly. The script now names the prefix class (never the key) and refuses the private copy on a publishable one; the next dispatch prints which it is.

**Measured a third time (run 35893779483, 17:12Z, NAS checkout 8e92216):** `hosted key shape: {"len": 57, "prefix3": "cdn", "dots": 0, "quoted": false, "whitespace": false}`. The value in `HOSTED_SERVICE_ROLE_KEY` is not a Supabase API key of any family — a legacy key is a JWT (`eyJ…`, two dots, about 200 characters) and a new key starts `sb_`. Whatever was pasted came from a different field. The script now names this case (`not-a-key`), withholds the private buckets with the two keys that do work spelled out, and no longer spends 322 downloads finding out. The paste that closes DR-0317 is the `service_role` value from Project Settings › API Keys › Legacy (starts `eyJ`), or the Secret key from API Keys (starts `sb_secret_`).
