...
- **"Inside the Love Corner App also"** (Darrell, repeated): the Admin panel was scoped to the one instance the shell passes (family/personal). Migration `0112` adds `list_my_admin_instances()` (the caller's own owner/admin spaces, church-first), and the **Manage access roles** panel now shows an **instance picker** so a church leader targets the COLG/Love Corner instance (or any ministry space they lead), not only the family space. Isolation-tested in the same smoke (owner/admin see only their own admin spaces; a member sees none).
