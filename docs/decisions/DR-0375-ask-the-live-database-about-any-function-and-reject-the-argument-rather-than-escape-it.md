...
- ~~**The instrument has not been dispatched against the sovereign database.**~~ **DONE, same day, on its first real use.** `sovereign-read` run 34739384541 (`definitions`, `functions=crm_capture_lead`) answered from the live database: `source_md5` = `d22ffd311f16111e75f070f1314a51c5`, **identical to the hosted value**, `---MISSING--- none`, `sovereign_replay=221`. That closed DR-0374's remaining migration gap by measurement rather than by the replay step's word — which is the entire reason this instrument exists, proven rather than argued.
