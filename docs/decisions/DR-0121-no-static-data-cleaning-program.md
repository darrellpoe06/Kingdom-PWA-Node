...
| 3 | Projects → Discussions | Zero records; "should be full of Ari notes" | First standing assignment of the Local-LLM tending lane (DR-0120 §3): an Ari note (kind reflection/decision, `links.dr_ref`) per shipped feature. Constraint (verified): the build agent cannot write family-instance rows (RLS, by design) — so this rides the credentialed local lane, three brakes, Tier C to arm | OPEN — `re-review: 2026-07-14` |
...
| 5 | Projects → PoeTech Build | Hand-typed ROADMAP frozen at 2026-06-17 (20 days); 8 past target; copy claimed "not hand-typed" | NOW: record declares its own age (staleness read-out shipped with this DR) + copy corrected. NEXT: consolidate the ship story onto the LIVE board store / seed specs — one maintained source; retire the duplicate constant | Read-out **SHIPPED**; consolidation OPEN — `re-review: 2026-07-11` |
...
- The BuildBoard consolidation (item 5) is the next build increment in this program; its landing closes the last known hand-typed work-record in the app.
- Items 3, 4, and 6's data-cleaning halves land with the Local-LLM tending lane (DR-0120 §3) — arming that lane is the program's Tier-C milestone.
