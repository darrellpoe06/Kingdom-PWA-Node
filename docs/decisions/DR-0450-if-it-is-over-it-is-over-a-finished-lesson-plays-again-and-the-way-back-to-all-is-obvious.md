...
- **Measured in real Chromium at phone width (2026-09-17, after the merge).** The way back to ALL is **158x44px at 13px, filled**, and the lesson bar stays **one row at 58px**; at Big Print the bar's buttons measure **57px**, under the 64px chrome cap, so the words grew and the frame did not. The two end doors are **336x48px each** at Normal and 46px at Big Print (the chrome cap compressing them), both above the 44px touch floor. **The floor is now a gate:** `scripts/chrome-layout-probe.mjs` carries `WAY_OUT_MIN_PX` 44 and `WAY_OUT_MIN_FONT_PX` 12 beside the existing ceiling, and asserts the way out exists, says ALL, and is the bar's filled primary control. A ceiling without a floor was half a guarantee — nothing had stopped the control from shrinking back. **Proven to catch:** restoring the old 10px/36px/hairline button fails the probe three ways at every width, naming both old numbers. The live pass on his own device is still the DR-0104 step.
