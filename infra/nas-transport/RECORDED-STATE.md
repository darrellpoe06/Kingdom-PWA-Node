...
| `/voice` | the voice studio that ANSWERED this cycle — `http://127.0.0.1:8770`, else `http://tlcmediadpt:8770` (the church 4070, `infra/church-gpu-node` `voice-studio`) | the sovereign reading voice (DR-0138/DR-0382/DR-0394) — `POST /voice/speak`, `GET /voice/health`; the same-origin Pages Function `/voice/*` proxies here | intended state added 2026-09-20; actuated idempotently by `infra/voice-studio/install.sh` via services-sync. The backend is DISCOVERED rather than fixed, because unlike every row above this one the service does not live on this box: it is a GPU container on the 4070, armed by `.github/workflows/arm-voice-studio.yml`. The installer mounts ONLY a host that has already answered `/health` on that cycle — a path mounted at a dark host is worse than no path, since it hangs the reader instead of failing it and reads as actuated in every check. The server answers to BOTH `/speak` and `/voice/speak` (the `/taxes` precedent) so the mount cannot be wrong about prefix stripping, which nothing here can measure. NOTE (honest history): the app's `/voice` transport and this row were written in the SAME merge — the first route in this file not to repeat the `/nas-photos`/`/taxes` pattern of shipping the client hop weeks before the road home, and it was `scripts/funnel-actuation-guard.mjs` that refused to let it. |
