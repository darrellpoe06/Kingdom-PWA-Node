...
| `/voice` | `http://127.0.0.1:8770` (the voice studio, when it answers on THIS box) | the sovereign reading voice (DR-0138/DR-0382/DR-0394) -- `POST /speak`, `GET /health`; the same-origin Pages Function `/voice/*` proxies here, and tailscale STRIPS the `/voice` mount point before forwarding, so the server sees the bare paths (it answers to the prefixed spellings too, which costs nothing). | intended state added 2026-09-20; actuated idempotently by `infra/voice-studio/install.sh` via services-sync, which mounts ONLY a backend that has already answered `/health` on that cycle -- a path mounted at a dark host hangs the reader instead of failing them and reads as actuated in every check. **HONEST GAP, stated rather than hidden:** the studio normally runs on the 4070 (`tlcmediadpt`), not on the NAS, and tailscale refuses a non-local proxy target outright -- *"only localhost or 127.0.0.1 proxies are currently supported"*, tailscale/tailscale#8751, open since 2023-07-31. This row was first written naming the GPU box by MagicDNS, which would have failed on every cycle while this table and the actuation guard both read green. Reaching a studio on the 4070 needs a LOCAL forwarder on the NAS -- the same 127.0.0.1 shape every row above already has -- carrying the bearer check those services have, because `/voice` is on the PUBLIC Funnel and the studio has no authentication of its own. Not built yet. re-review: 2026-09-21. |
